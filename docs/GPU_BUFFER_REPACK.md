...
- `gpu_repack_commit_ready`
- `gpu_repack_commit_steps`
- `gpu_repack_commit_tail_free`
- `gpu_repack_apply_ready`
- `gpu_repack_apply_steps`
- `gpu_repack_apply_slots`
- `gpu_repack_ms`
...
The current replacement-upload, binding-preview, draw-remap-preview, staged-guard, commit-proof, and apply-scaffold slice assembles a compact replacement byte payload from the resident source map in deterministic plan order, builds replacement indirect draw command bytes for the current draw order using the compacted `first_instance` offsets, then collects the replacement face bytes, draw bytes, deterministic slot map, and tail free range into one all-or-nothing staged swap preview. A disabled commit proof validates the future operation order: face-buffer RID swap, render binding swap, indirect draw buffer swap, slot-map replacement, and allocator rebuild. A disabled apply preview then consumes that proof and verifies that the staged bytes, slots, free ranges, and operation order still match before reporting apply readiness. With the additional explicit `RUMPELMC_GPU_TERRAIN_BUFFER_REPACK_UPLOAD_PREVIEW=1` profiling flag, the runtime takes one sample: it creates a temporary replacement storage buffer, uploads the staged face bytes, creates and validates a temporary uniform set that binds the replacement face buffer to the existing shader/atlas resources, then immediately frees the temporary uniform set and buffer. Telemetry reports `gpu_repack_payload_ready`, `gpu_repack_payload_bytes`, `gpu_repack_upload_ready`, `gpu_repack_upload_bytes`, `gpu_repack_upload_ms`, `gpu_repack_bind_ready`, `gpu_repack_bind_ms`, `gpu_repack_draw_ready`, `gpu_repack_draw_bytes`, `gpu_repack_stage_ready`, `gpu_repack_stage_slots`, `gpu_repack_stage_bytes`, `gpu_repack_commit_ready`, `gpu_repack_commit_steps`, `gpu_repack_commit_tail_free`, `gpu_repack_apply_ready`, `gpu_repack_apply_steps`, and `gpu_repack_apply_slots`; source size mismatches report `gpu_repack_failure_reason=source_size_mismatch`, invalid temporary buffer creation reports `gpu_repack_failure_reason=upload_error`, and temporary binding, draw-remap, staged draw-size, slot-map, tail-range, commit draw-size, or apply-proof failure reports `gpu_repack_failure_reason=draw_rebuild_error`. Runtime buffer replacement, active render binding swap, indirect-buffer upload/swap, slot mutation, and allocator mutation are still disabled.
...
The next slice should add a final-swap disabled runtime guard that can keep apply readiness separated from any active RenderingDevice RID, slot-map, indirect-buffer, or allocator mutation. The final RenderingDevice buffer swap must stay disabled until upload, swap, telemetry, and abort paths are covered by tests.
