...
- CLI table inspection showed generic Godot/Xcode labels such as command-buffer render/blit commands, drawable present events, completion handlers, and shader entries, but no stable shadow-pass label or mapping. Do not infer `gpu_shadow_pass_ms` from those rows.
