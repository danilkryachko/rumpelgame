...
- `logs/external_profiling_campaign_current/external-profiling-campaign-summary.txt` now also reports `xctrace_review_capture_status=present`, `xctrace_review_packet_status=pass`, and `xctrace_review_packet_check_status=pass` for the current local capture directory.
