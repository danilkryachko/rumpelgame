...
| --- | --- | --- | --- | --- |
| `post-72395e8` | Flyback chunk-hole stress | `logs/gpu_flyback_stress` | release fly-out/fly-back smoke passed with `motion_steps=13`, `motion_chunks=7`, `current_chunk_loaded=1`, `current_chunk_submeshes=2`, `current_chunk_collision=2`, `ground_hit=1`, `ground_distance=12.000`, `terrain_samples=381`, `terrain_queue_max_ms=2.453`, `gpu_compositor_submit_max_ms=0.228`, and `gpu_upload_fail=0` | correctness gate; performance budgets are report-only by default |
