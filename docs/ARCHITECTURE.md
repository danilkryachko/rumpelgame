...
2. `server/cmd/server/main.go` opens RocksDB, creates `world.World`, creates `network.Server`, loads persisted item entities, and starts the TCP listener.
...
9. The local player starts in first-person mode; pressing `V` toggles the Rust-owned third-person camera and shows the Rust-built modular voxel character visual without changing server reach validation or protocol state.
...
- Server client sessions own the placement inventory and player-body collision boundary for `BlockAction_PLACE`, counted/survival target-block mining cooldown for `BlockAction_DESTROY`, server-owned item entity spawn/merge/despawn for destroyed placeable blocks, persisted uncollected item entities, and validated item pickup into inventory.
- `World.SetBlockGlobal` applies block edits and persists dirty chunks through the configured `ChunkStore`.
- The Go server passes the same RocksDB store to the network layer for local-player inventory persistence under a separate player-inventory key prefix and item-entity persistence under a separate item-entity key prefix.
...
- Current packet payload variants are `ChunkData`, `ClientPosition`, `BlockAction`, `InventorySnapshot`, `InventoryAction`, `ItemEntitySnapshot`, and `ItemPickupAction`.
...
- Inventory placement remains on the current `BlockAction` packet shape, server-to-client inventory slots plus selected block/tool slots use `Packet.inventory_snapshot = 4`, client selected-slot and selected-tool requests use `Packet.inventory_action = 5`, item entity snapshots use `Packet.item_entities = 6`, item pickup requests use `Packet.item_pickup = 7`, and `ClientPosition.player_id = 4` carries the optional stable local-player identity used for inventory persistence; selected tool slot is session state in this checkpoint and does not change the RocksDB player-inventory record. Additional inventory packets must use new `Packet.payload` tags and pass the inventory protocol compatibility gate.
...
- Local creative hotbar input state includes the requested selected slot and selected block ID, and local tool input maps bounded tool slots to `InventoryAction_SELECT_TOOL_SLOT`. Server sessions own creative placement inventory, selected-slot validation for `InventoryAction_SELECT_SLOT`, selected-tool validation for `InventoryAction_SELECT_TOOL_SLOT`, last-position reach validation for `BlockAction`, player-body collision rejection for `BlockAction_PLACE`, placement approval for `BlockAction_PLACE`, counted/survival target-block mining cooldown for `BlockAction_DESTROY`, selected-tool mining adjustment through server-owned item/tool IDs, item entity spawn/merge/despawn for destroyed placeable blocks, item-entity restart persistence, validated pickup collection through `ItemPickupAction`, and local-player inventory load/save when `ClientPosition.player_id` is valid; the Rust client copies authoritative inventory slots, selected slot, and selected tool slot from `InventorySnapshot`, copies authoritative item entities from `ItemEntitySnapshot`, renders runtime item meshes, requests nearby pickups, and the Godot HUD displays authoritative labels/counts/tool text. Persistent block edits flow through `World.SetBlockGlobal` or `World.ReplaceBlockGlobal`.
- The local `Player` owns first-person and third-person camera placement, keeps the local player scale at roughly `1.90m` with a `1.80m` eye/camera height, extends the client block raycast only by the third-person camera offset, builds a visible third-person-only modular voxel character through the Rust `biomes_avatar` path from checked-in Biomes MagicaVoxel `.vox` assets, supports local skin/eye/hair appearance preset cycling, exposes all 44 Biomes source animation clips for HUD preview, and applies movement plus grouped Biomes clip preview poses directly to those pivots.
