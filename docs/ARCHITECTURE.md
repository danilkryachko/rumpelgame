...
- The local `Player` owns first-person and third-person camera placement, keeps the local player scale at roughly `1.90m` with a `1.80m` eye/camera height, extends the client block raycast only by the third-person camera offset, builds a visible third-person-only modular voxel character from Veloren-derived MagicaVoxel `.vox` body parts loaded by the Rust GDExtension, and applies procedural `idle`/`run`/`jump` poses directly to those pivots.
