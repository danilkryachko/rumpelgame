...
- Server client sessions own the placement inventory and player-body collision boundary for `BlockAction_PLACE`, counted/survival target-block mining cooldown for `BlockAction_DESTROY`, and counted break-drop insertion for `BlockAction_DESTROY`.
...
- Inventory placement remains on the current `BlockAction` packet shape, server-to-client inventory slots plus selected block/tool slots use `Packet.inventory_snapshot = 4`, client selected-slot and selected-tool requests use `Packet.inventory_action = 5`, and `ClientPosition.player_id = 4` carries the optional stable local-player identity used for inventory persistence; selected tool slot is session state in this checkpoint and does not change the RocksDB player-inventory record. Additional inventory packets must use new `Packet.payload` tags and pass the inventory protocol compatibility gate.
...
- Local creative hotbar input state includes the requested selected slot and selected block ID, and local tool input maps bounded tool slots to `InventoryAction_SELECT_TOOL_SLOT`. Server sessions own creative placement inventory, selected-slot validation for `InventoryAction_SELECT_SLOT`, selected-tool validation for `InventoryAction_SELECT_TOOL_SLOT`, last-position reach validation for `BlockAction`, player-body collision rejection for `BlockAction_PLACE`, placement approval for `BlockAction_PLACE`, counted/survival target-block mining cooldown for `BlockAction_DESTROY`, selected-tool mining adjustment through server-owned item/tool IDs, counted break-drop insertion for `BlockAction_DESTROY`, and local-player inventory load/save when `ClientPosition.player_id` is valid; the Rust client copies authoritative inventory slots, selected slot, and selected tool slot from `InventorySnapshot`, and the Godot HUD displays those authoritative labels/counts/tool text. Persistent block edits flow through `World.SetBlockGlobal` or `World.ReplaceBlockGlobal`.
