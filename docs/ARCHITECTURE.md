...
- Server client sessions own the placement inventory and player-body collision boundary for `BlockAction_PLACE`, counted/survival target-block mining cooldown for `BlockAction_DESTROY`, server-owned item entity spawn for destroyed placeable blocks, persisted uncollected item entities, and validated item pickup into inventory.
