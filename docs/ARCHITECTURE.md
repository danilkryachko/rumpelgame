...
- Current generation uses an explicit `GeneratorConfig` with `seed`, `dimension_id`, and `version=flat_v1`; server startup validates `RUMPELMC_WORLD_SEED`, `RUMPELMC_WORLD_DIMENSION_ID`, and `RUMPELMC_WORLD_GENERATOR_VERSION` before creating `World`. The default `flat_v1` path still produces the deterministic flat terrain byte contract. Biome/quality layers are documented but not active runtime behavior.
