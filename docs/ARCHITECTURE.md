...
- The local `Player` owns first-person and third-person camera placement, extends the client block raycast only by the third-person camera offset, builds a visible third-person-only modular voxel character from Veloren-derived MagicaVoxel `.vox` body parts loaded by the Rust GDExtension, and applies procedural `idle`/`run`/`jump` poses directly to those pivots.
