...
- Go RLE tests also cover representative opt-in `height_v1` and `biome_height_v1` chunks, proving generated terrain decodes back to the exact raw serialized bytes.
...
- Focused deterministic packet/RLE property coverage, including representative `height_v1` and `biome_height_v1` RLE round-trip coverage, is surfaced by the security gate as `deterministic_property_tests=guarded`.
...
- Focused deterministic packet/RLE property tests are present in Go and Rust test sources, including representative `height_v1` and `biome_height_v1` RLE round-trip coverage, and surfaced in the summary as `deterministic_property_tests=guarded`.
- Focused Go protocol/network/storage/world tests pass.
