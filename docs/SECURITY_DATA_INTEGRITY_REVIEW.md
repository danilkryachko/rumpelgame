...
- Server write deadlines, failed interested-client broadcast cleanup, bounded connection lifecycle log summaries, bounded slow-reader timeout evidence, and bounded six-client fanout/load/resource/per-client detail evidence are guarded by the networking and server scalability gates.
