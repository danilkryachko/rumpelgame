...
- Server world/network tests prove out-of-range block-edit `Y` is rejected without a save or chunk broadcast, and session block edits are ignored before a recorded position or beyond the server reach envelope.
