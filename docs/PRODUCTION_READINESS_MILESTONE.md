...
- `logs/external_profiling_campaign_current/external-profiling-campaign-summary.txt` reported `status=pass`, `reason=external_profiler_pending`, `capture_readiness=live_rc_ready_for_external_capture`, `external_profile_status=pending_external_profiler`, `results_file_status=missing`, `results_template_status=todo`, `results_check_status=missing`, `xctrace_review_packet_status=pass`, `captured_rows=0`, `missing_rows=4`, and `rc_live_checks=full`; `external-profiling-results-intake.txt` is an operator intake contract, and the xctrace review packet is a navigation artifact, not profiler evidence.
