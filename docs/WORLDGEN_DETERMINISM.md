...
- `TestConfiguredBiomeCaveHeightV1GeneratorIsDeterministicForSeedDimensionAndCoordinates` proves opt-in `biome_cave_height_v1` chunk bytes are deterministic, preserve biome-selected surface columns, carve deterministic underground openings, and have representative SHA-256 `c781f5530436094665f9596b9338065717400f4d497b9dd32f3f0d8839ff76a0`.
- `TestConfiguredBiomeCaveHeightV1GeneratorChangesWithSeedAndDimension` proves opt-in biome+cave generated bytes respond to seed and dimension inputs.
- `TestEncodeSerializedChunkRLERoundTripsBiomeCaveHeightV1Chunk` proves a representative `biome_cave_height_v1` chunk round-trips through the existing RLE codec.
- `scripts/biome_cave_height_generator_matrix_gate.sh` guards representative `biome_cave_height_v1` bytes with `carved_air=28152`, `surface_preserved=1024`, `biome_surface_changed=1024`, and no protocol diff.
