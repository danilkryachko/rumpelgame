...
- `scripts/storage_package_smoke.sh` runs the storage/server-config smoke with dummy PostgreSQL environment variables and no external secrets.
...
- The same RocksDB store is passed to `network.NewServerWithPlayerInventoryStore` so player inventory state and chunk data share the approved backend while keeping separate key prefixes.
