...
- For gameplay loop foundation checks, use `scripts/player_inventory_reconnect_smoke.sh` before `scripts/gameplay_loop_foundation_gate.sh`; see `docs/GAMEPLAY_LOOP_FOUNDATION.md`. Current expected status is `pass` with local hotbar inventory, selected slot state, server session inventory guarded, `server_inventory_persistence=rocksdb_guarded`, `player_inventory_reconnect=live_server_guarded`, required Block 41 persisted visual proof, `full_reload_persistence=block_41_visual_guarded`, and no protocol diff.
