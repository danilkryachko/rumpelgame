...
The storage foundation slice keeps RocksDB as the implemented chunk persistence backend and does not add PostgreSQL behavior. It adds focused coverage for missing chunks, save/reopen round-trips, overwrite isolation between neighboring chunk keys, concurrent distinct-key save/load, corrupt persisted payload errors, missing-parent path creation, regular-file path rejection, and the server config boundary that keeps `RUMPELMC_SERVER_ROCKSDB_PATH` as the only current runtime chunk-store path override.

Fresh check:

- `go test ./pkg/storage` and `go test -race ./pkg/storage` passed on 2026-06-16 with the concurrent distinct-key save/load coverage.
...
- RocksDB chunk persistence is guarded by focused `server/pkg/storage` tests for key format, sorted signed coordinate keys, round-trip save/load, missing chunks, overwrite isolation, concurrent distinct-key save/load, and corrupt payload errors.
