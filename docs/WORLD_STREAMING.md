...
- For release-candidate evidence checks, use `scripts/release_candidate_gate.sh`; see `docs/RELEASE_CANDIDATE_GATE.md`. Current expected status is `pass` with `security_deterministic_property_tests=guarded`; current quick artifacts can report `live_checks=skipped`, and a real release branch must rerun the gate with live fast/full/diff flags.
...
- For the production-readiness milestone, use `scripts/production_readiness_milestone_gate.sh`; see `docs/PRODUCTION_READINESS_MILESTONE.md`. Current expected status is `pass` with `production_readiness=rc_evidence_ready`, `rc_security_deterministic_property_tests=guarded`, `external_capture_readiness=ready_for_external_capture`, and current quick `live_release_checks=skipped`, while external profiler results, a final live RC run, native shadow, and active transparent terrain remain deferred.
