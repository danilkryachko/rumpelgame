...
- For biome and visual-variety foundation, use `scripts/biome_visual_variety_foundation_gate.sh`; see `docs/BIOME_VISUAL_VARIETY_FOUNDATION.md`. Current expected status is `pass` with `biome_sampler=guarded`, `biome_matrix=guarded`, metadata-only runtime ownership, runtime biome visuals deferred, and no worldgen/serialization change.
- For world generation quality planning, use `scripts/world_generation_quality_gate.sh`; see `docs/WORLD_GENERATION_QUALITY_PASS.md`. Current expected status is `pass` with `worldgen_seed_version=guarded`, `worldgen_height_v1=guarded`, `height_v1_serialization=guarded`, `height_v1_live_smoke=guarded`, `biome_sampler=guarded`, `biome_matrix=guarded`, `origin_chunk=guarded`, `flat_byte_hash=guarded`, `coordinate_mapping=guarded` for positive, negative, and high-positive boundaries, and `runtime_quality_pass=opt_in_height_v1_guarded`; cave/resource/structure/biome runtime and default-world terrain changes remain blocked.
