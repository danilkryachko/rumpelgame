...
- For gameplay loop foundation checks, use `scripts/gameplay_loop_foundation_gate.sh`; see `docs/GAMEPLAY_LOOP_FOUNDATION.md`. Current expected status is `pass` with local hotbar inventory guarded, required Block 41 persisted visual proof, `full_reload_persistence=block_41_visual_guarded`, and no protocol diff.
