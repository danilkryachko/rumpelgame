...
- For release-candidate evidence checks, use `scripts/release_candidate_gate.sh`; see `docs/RELEASE_CANDIDATE_GATE.md`. Current expected status is `pass` with `security_deterministic_property_tests=guarded` and `security_local_server_exposure=loopback_default_guarded`; current quick artifacts can report `live_checks=skipped`, and a real release branch must rerun the gate with live fast/full/diff flags.
