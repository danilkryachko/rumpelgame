...
- For release-candidate evidence checks, use `scripts/release_candidate_gate.sh`; see `docs/RELEASE_CANDIDATE_GATE.md`. Current expected status is `pass` with `security_deterministic_property_tests=guarded`; latest current artifact reports `live_checks=full`, `fast_check=pass`, `full_check=pass`, `diff_check=pass`, and `diff_guard=pass`, with Go lint skipped inside `check.sh full` because `golangci-lint` is unavailable.
