...
The client also has a guarded third-person character visual slice. `Player` keeps first-person as the default, toggles third-person camera mode with `V`, shows `res://kenney_character_preview.tscn` only in third-person mode, and sends stable `idle`/`run`/`jump` motion-state labels to the Godot character script. This is client presentation only: it does not change packet schema, server reach validation, storage, world generation, chunk serialization, GPU terrain renderer policy, draw distance, lighting, shadows, or texture quality.
