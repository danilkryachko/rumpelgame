...
- For the production-readiness milestone, use `scripts/production_readiness_milestone_gate.sh`; see `docs/PRODUCTION_READINESS_MILESTONE.md`. Current expected status is `pass` with `production_readiness=rc_evidence_ready`, `rc_security_deterministic_property_tests=guarded`, and current quick `live_release_checks=skipped`, while external profiler, a final live RC run, native shadow, and active transparent terrain remain deferred.
