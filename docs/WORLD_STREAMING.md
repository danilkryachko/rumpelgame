...
See `docs/SERVER_INVENTORY_FOUNDATION.md` for the 2026-06-16 server-owned session inventory placement checkpoint and the 2026-06-17 counted runtime plus break-drop insertion evidence.

See `docs/INVENTORY_PROTOCOL_COMPATIBILITY.md` for the 2026-06-16 inventory protocol compatibility checkpoint over the current `BlockAction` packet shape, server-to-client inventory snapshot, and client-to-server selected-slot action.
...
Inventory protocol compatibility is guarded with a server-to-client snapshot packet and a client-to-server selected-slot action packet. The current wire contract uses `Packet.block_action = 3` plus `BlockAction.block_id = 5` for placement requests, `Packet.inventory_snapshot = 4` for authoritative inventory slots plus selected slot, and `Packet.inventory_action = 5` for selected-slot requests. Server session inventory remains the placement approval, player-body placement rejection, destroy-drop insertion, and selected-slot validation boundary; `RUMPELMC_SERVER_INVENTORY_MODE=counted` uses the same protocol and persists decremented/added finite counts through the existing player-inventory record. The compatibility gate requires clean protocol drift, server inventory, and gameplay foundation evidence.
...
- For server inventory foundation checks, use `scripts/player_inventory_counted_smoke.sh` and `scripts/player_inventory_break_drop_smoke.sh` before `scripts/server_inventory_foundation_gate.sh`; see `docs/SERVER_INVENTORY_FOUNDATION.md`. Current expected status is `pass` with `server_inventory_status=session_guarded`, `creative_inventory=unit_guarded`, `counted_inventory=unit_guarded`, `counted_inventory_runtime=live_server_guarded`, `break_drop_inventory=live_server_guarded`, `block_action_inventory=session_guarded`, `player_inventory_persistence=rocksdb_guarded`, and no protocol or storage diff.
