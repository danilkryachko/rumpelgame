...
- For world generation quality planning, use `scripts/world_generation_quality_gate.sh`; see `docs/WORLD_GENERATION_QUALITY_PASS.md`. Current expected status is `pass` with `worldgen_seed_version=guarded`, `origin_chunk=guarded`, `flat_byte_hash=guarded`, `coordinate_mapping=guarded` for positive, negative, and high-positive boundaries, and runtime terrain/cave/resource/structure changes deferred.
