...
Inventory protocol compatibility is guarded with a server-to-client snapshot packet and a client-to-server selected-slot action packet. The current wire contract uses `Packet.block_action = 3` plus `BlockAction.block_id = 5` for placement requests, `Packet.inventory_snapshot = 4` for authoritative inventory slots plus selected slot, and `Packet.inventory_action = 5` for selected-slot requests. Server session inventory remains the placement approval, destroy-drop insertion, and selected-slot validation boundary; `RUMPELMC_SERVER_INVENTORY_MODE=counted` uses the same protocol and persists decremented/added finite counts through the existing player-inventory record. The compatibility gate requires clean protocol drift, server inventory, and gameplay foundation evidence.
