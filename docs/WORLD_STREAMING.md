...
- For release-candidate evidence checks, use `scripts/release_candidate_gate.sh`; see `docs/RELEASE_CANDIDATE_GATE.md`. Current expected status is `pass` with `security_deterministic_property_tests=guarded`, `security_conflict_semantics=last_write_wins_guarded`, `security_local_server_exposure=loopback_default_guarded`, and `security_smoke_bind_exposure=loopback_guarded`; current quick artifacts can report `live_checks=skipped`, and a real release branch must rerun the gate with live fast/full/diff flags.
- For external profiler campaign checks, use `scripts/external_profiling_campaign_gate.sh`; see `docs/EXTERNAL_PROFILING_CAMPAIGN.md`. Current expected status is `pass` with `capture_readiness=ready_for_external_capture`, `rc_security_deterministic_property_tests=guarded`, `rc_security_conflict_semantics=last_write_wins_guarded`, `rc_security_local_server_exposure=loopback_default_guarded`, `rc_security_smoke_bind_exposure=loopback_guarded`, and `external_profile_status=pending_external_profiler`; use `scripts/gpu_terrain_shadow_xctrace_attach_capture.sh` for sanitized local macOS attach attempts, but do not cite the pending capture pack, results-intake file, TODO template, raw trace, or exported Metal tables as measured profiler evidence.
- For the production-readiness milestone, use `scripts/production_readiness_milestone_gate.sh`; see `docs/PRODUCTION_READINESS_MILESTONE.md`. Current expected status is `pass` with `production_readiness=rc_evidence_ready`, `rc_security_deterministic_property_tests=guarded`, `security_conflict_semantics=last_write_wins_guarded`, `security_local_server_exposure=loopback_default_guarded`, `security_smoke_bind_exposure=loopback_guarded`, `rc_security_conflict_semantics=last_write_wins_guarded`, `rc_security_local_server_exposure=loopback_default_guarded`, `rc_security_smoke_bind_exposure=loopback_guarded`, `external_rc_security_conflict_semantics=last_write_wins_guarded`, `external_rc_security_local_server_exposure=loopback_default_guarded`, `external_rc_security_smoke_bind_exposure=loopback_guarded`, `external_capture_readiness=ready_for_external_capture`, and current quick `live_release_checks=skipped`, while external profiler results, a final live RC run, native shadow, and active transparent terrain remain deferred.
