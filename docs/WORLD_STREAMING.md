...
The client also has a guarded third-person character visual slice. `Player` keeps first-person as the default, uses a roughly `1.90m` local player height with `1.80m` eye/camera height, toggles third-person camera mode with `V`, shows the Rust-built `PlayerVoxelCharacter` only in third-person mode, composes Veloren-derived voxygen RON manifests and MagicaVoxel `.vox` parts with GPL attribution under `client/assets/veloren/`, and applies procedural idle/run/jump poses to modular body-part pivots. This is client presentation only: it does not change packet schema, server reach validation, storage, world generation, chunk serialization, GPU terrain renderer policy, draw distance, lighting, shadows, or texture quality.
