...
- For block material metadata design and the existing-ID server/client registry foundations, use `scripts/block_material_metadata_design_gate.sh`, `scripts/block_material_registry_foundation_gate.sh`, and `scripts/client_block_material_registry_foundation_gate.sh`; see `docs/BLOCK_MATERIAL_METADATA_DESIGN.md`. Current expected status is `pass` with `production_metadata_status=server_registry_guarded`, `server_material_metadata=guarded`, `client_material_metadata=guarded`, `active_schema_change=0`, and the runtime contract still opaque-only.
