...
See `docs/SERVER_INVENTORY_FOUNDATION.md` for the 2026-06-16 server-owned session inventory placement checkpoint and the 2026-06-17 counted runtime plus item entity pickup evidence.

See `docs/INVENTORY_PROTOCOL_COMPATIBILITY.md` for the 2026-06-17 inventory protocol compatibility checkpoint over the current `BlockAction` packet shape, server-to-client inventory and item entity snapshots, client-to-server selected-slot/tool-slot actions, and item pickup action.
...
See `docs/STORAGE_PERSISTENCE_FOUNDATION.md` for the 2026-06-16 RocksDB chunk and local-player inventory persistence guard.
...
The current mining/building loop remains `Player` raycast signal -> `GameClient` `BlockAction` -> server session inventory/reach/player-body validation -> server world edit boundary -> updated chunk snapshot. Placement uses `World.SetBlockGlobal`; connected-session destroy uses `World.ReplaceBlockGlobal` so counted inventory can receive the destroyed placeable block after a successful edit. Counted/survival destroy actions are also guarded by a server-owned target-block mining cooldown before world mutation, and the server-owned item/tool foundation can adjust that duration through the selected `tool:hand` or wooden tool while explicit global mining overrides stay exact; default creative sessions keep zero mining cooldown. The client now has a local creative hotbar inventory model with explicit slot counts, selected slot state, and placeability validation, while the Godot HUD displays server-authoritative hotbar labels/counts and selected-slot highlight from `InventorySnapshot`. The server now has a session-owned creative inventory placement boundary that preserves current creative behavior while rejecting blocks absent from the session inventory, beyond the last recorded client position reach envelope, or intersecting the last recorded player body AABB. Full save -> reload -> visual/collision/GPU verification is supplied by the completed Block 41 persisted visual chain and required by the gameplay foundation gate.

The client also has a guarded third-person character visual slice. `Player` keeps first-person as the default, uses a roughly `1.90m` local player height with `1.80m` eye/camera height, toggles third-person camera mode with `V`, cycles local Biomes-style appearance presets with `B`, exposes the HUD character creator with `C`, shows the Rust-built `PlayerVoxelCharacter` only in third-person mode, composes Biomes-derived MagicaVoxel `.vox` parts with MIT attribution under `client/assets/biomes/`, keeps the Biomes source character animation GLTF alongside the avatar metadata, exposes all 44 Biomes source animation clips for local preview, and applies movement plus grouped clip preview poses to modular body-part pivots. This is client presentation only: it does not change packet schema, server reach validation, storage, world generation, chunk serialization, GPU terrain renderer policy, draw distance, lighting, shadows, or texture quality.

Fresh check:

- `logs/mining_rules_foundation_current/mining-rules-foundation-summary.txt` should report `status=pass`, `mining_rules_status=cooldown_guarded`, `mining_block_durations=target_block_guarded`, `creative_default=unchanged`, `counted_mining_cooldown=server_guarded`, `active_protocol_change=0`, and `go_tests=pass`.
- `logs/item_tool_foundation_current/item-tool-foundation-summary.txt` should report `status=pass`, `item_tool_foundation=server_guarded`, `item_identity=block_items_guarded`, `tool_catalog=wood_tools_guarded`, `first_tool_slot=hand_guarded`, `tool_mining=selected_tool_guarded`, `active_protocol_change=0`, `active_storage_change=0`, `active_worldgen_change=0`, and `go_tests=pass`.
- `logs/gameplay_loop_foundation_current/gameplay-loop-foundation-summary.txt` should report `status=pass`, `inventory_tests=pass`, `server_tests=pass`, `hotbar_selection=unit_guarded`, `inventory_hud=authoritative_guarded`, `tool_hud=authoritative_guarded`, `item_entity_pickup=live_server_guarded`, `server_inventory_status=session_guarded`, `server_inventory_block_action=session_guarded`, `server_inventory_persistence=rocksdb_guarded`, `mining_rules_status=cooldown_guarded`, `mining_cooldown=server_guarded`, `mining_block_durations=target_block_guarded`, `item_tool_foundation=server_guarded`, `item_identity=block_items_guarded`, `tool_catalog=wood_tools_guarded`, `first_tool_slot=hand_guarded`, `tool_mining=selected_tool_guarded`, `player_inventory_reconnect=live_server_guarded`, `server_edit_persistence=store_save_boundary`, `full_reload_persistence=block_41_visual_guarded`, `block_edit_visual_path=godot_persisted_reload_guarded`, and `active_protocol_change=0` after this slice is committed.
- `logs/third_person_character_visual_current/third-person-character-visual-summary.txt` should report `status=pass`, `third_person_visual=guarded`, `camera_toggle=rust_guarded`, `character_visual=biomes_avatar_guarded`, `character_creator=godot_hud_guarded`, `character_assets=biomes_vox_guarded`, `character_asset_license=mit_attributed`, `character_animation=biomes_44_clip_preview_guarded`, `biomes_avatar=palette_wearable_guarded`, `vox_loader=magica_vox_guarded`, `rust_tests=pass`, `godot_smoke=pass`, `active_protocol_change=0`, `active_storage_change=0`, and `active_worldgen_change=0` after this slice is committed.

## Inventory Protocol Compatibility

Inventory protocol compatibility is guarded with server-to-client inventory snapshots, client-to-server selected-slot/tool-slot actions, server-to-client item entity snapshots, and client-to-server item pickup actions. The current wire contract uses `Packet.block_action = 3` plus `BlockAction.block_id = 5` for placement requests, `Packet.inventory_snapshot = 4` for authoritative inventory slots plus selected block/tool slots, `Packet.inventory_action = 5` for selected-slot/tool-slot requests, `Packet.item_entities = 6` for authoritative item entity snapshots, and `Packet.item_pickup = 7` for pickup requests. Server session inventory remains the placement approval, player-body placement rejection, destroy-drop item entity pickup, and selected-slot validation boundary; `RUMPELMC_SERVER_INVENTORY_MODE=counted` uses the same protocol and persists decremented/collected finite counts through the existing player-inventory record. The compatibility gate requires clean protocol drift, server inventory, and gameplay foundation evidence.

Fresh check:

- `logs/inventory_protocol_compatibility_current/inventory-protocol-compatibility-summary.txt` should report `status=pass`, `inventory_protocol_status=item_entity_pickup_guarded`, `active_schema_change=0`, `current_wire_contract=block_action_inventory_snapshot_action_tool_action_item_entity_pickup_and_player_id`, `position_identity_schema=tag_4_guarded`, `inventory_snapshot_schema=tag_4_guarded`, `inventory_action_schema=tag_5_guarded`, `tool_selection_schema=field_3_guarded`, `item_entity_snapshot_schema=tag_6_guarded`, `item_pickup_schema=tag_7_guarded`, `future_packet_tags=packet_gt_7`, `future_block_action_fields=block_action_gt_5`, `protocol_generated_drift=guarded`, `server_inventory_status=session_guarded`, and `gameplay_inventory_status=session_guarded` after this slice is committed. `ClientPosition.player_id = 4` is the current optional local-player identity field for inventory persistence.
...
- Gameplay foundation keeps mining/building on `BlockAction`, server session inventory, and `World.SetBlockGlobal`; completed Block 41 persisted visual/collision/GPU evidence is now a required gate input.
- Block edit persistence has a world-level save/reload guard for place and destroy edits, save-failure rollback, persisted load-error propagation, live restart/reload evidence, and a Godot persisted-reload visual/collision/GPU smoke matrix for place, destroy-after-reload, and chunk-edge edits.
...
- For biome and visual-variety foundation, use `scripts/biome_visual_variety_foundation_gate.sh`; see `docs/BIOME_VISUAL_VARIETY_FOUNDATION.md`. Current expected status is `pass` with `biome_sampler=guarded`, `biome_matrix=guarded`, `atlas_tile_identity=guarded`, `atlas_block_texture_usage=guarded`, metadata-only runtime ownership, default runtime biome visuals deferred, and no worldgen/serialization change in the foundation gate.
- For world generation quality planning, use `scripts/world_generation_quality_gate.sh`; see `docs/WORLD_GENERATION_QUALITY_PASS.md`. Current expected status is `pass` with `worldgen_seed_version=guarded`, `worldgen_height_v1=guarded`, `worldgen_biome_height_v1=guarded`, `worldgen_cave_height_v1=guarded`, `worldgen_biome_cave_height_v1=guarded`, `height_v1_serialization=guarded`, `biome_height_v1_serialization=guarded`, `cave_height_v1_serialization=guarded`, `biome_cave_height_v1_serialization=guarded`, `cave_distribution=guarded`, `resource_distribution=guarded`, `height_v1_live_smoke=guarded`, `biome_sampler=guarded`, `biome_matrix=guarded`, `biome_atlas_tile_identity=guarded`, `biome_atlas_block_texture_usage=guarded`, `cave_matrix=guarded`, `resource_matrix=guarded`, `origin_chunk=guarded`, `flat_byte_hash=guarded`, `coordinate_mapping=guarded` for positive, negative, and high-positive boundaries, and `runtime_quality_pass=opt_in_biome_cave_height_v1_guarded`; resource-to-block/structure/broader biome runtime and default-world terrain changes remain blocked.
- For server scalability checks, use `scripts/server_scalability_pass_gate.sh`; see `docs/SERVER_SCALABILITY_PASS.md`. Current expected status is `pass` with multi-client sent-state, interested-client fanout, `conflict_semantics=last_write_wins_guarded`, `chunk_request_ordering=guarded`, `nil_sent_state_policy=empty_guarded`, `view_distance_config=guarded`, propagated `worldgen_biome_atlas_tile_identity=guarded` and `worldgen_biome_atlas_block_texture_usage=guarded`, bounded classified connection lifecycle metrics, and bounded live/repeated multi-client evidence.
- For networking robustness checks, use `scripts/networking_robustness_gate.sh`; see `docs/NETWORKING_ROBUSTNESS_PROGRAM.md`. Current expected status is `pass` with Go/Rust packet boundary tests, zero-length frame decode, empty/unknown/nil-packet/nil-position/nil-block-action ignore policy, `scalability_status=repeat_live_guarded`, `resource_profile_status=repeat_live_guarded`, `multi_client_sent_state=guarded`, `block_edit_fanout=interested_clients_guarded`, `chunk_request_ordering=guarded`, `worldgen_biome_atlas_tile_identity=guarded`, `worldgen_biome_atlas_block_texture_usage=guarded`, `nil_sent_state_policy=empty_guarded`, `view_distance_config=guarded`, `disconnect_cleanup_status=lifecycle_summary_guarded`, zero connection close/accept failures, complete active-client lifecycle fields, required reconnect and slow-client runtime evidence, stale-session handling, packet-error classification, parser aggregation, live alert thresholds, `conflict_semantics=last_write_wins_guarded`, and opt-in admission guarded while adaptive overload/backpressure work remains deferred.
- For client state-machine checks, use `scripts/client_state_machine_hardening_gate.sh`; see `docs/CLIENT_STATE_MACHINE_HARDENING.md`. Current expected status is `pass` with lifecycle transitions unit-guarded, required reconnect smoke and bounded repeated reconnect soak evidence, `runtime_reconnect=repeated_live_rebootstrap_guarded`, `state_telemetry=live_marker_guarded`, `stale_packet_policy=session_guarded`, and no protocol diff.
- For server inventory foundation checks, use `scripts/player_inventory_counted_smoke.sh`, `scripts/player_inventory_break_drop_smoke.sh`, and `scripts/player_item_entity_persistence_smoke.sh` before `scripts/server_inventory_foundation_gate.sh`; see `docs/SERVER_INVENTORY_FOUNDATION.md`. Current expected status is `pass` with `server_inventory_status=session_guarded`, `creative_inventory=unit_guarded`, `counted_inventory=unit_guarded`, `counted_inventory_runtime=live_server_guarded`, `break_drop_inventory=live_server_guarded`, `item_entity_pickup=live_server_guarded`, `block_action_inventory=session_guarded`, `player_inventory_persistence=rocksdb_guarded`, and no protocol or storage diff.
- For item entity persistence checks, use `scripts/player_item_entity_persistence_smoke.sh` before `scripts/item_entity_persistence_gate.sh`; see `docs/ITEM_ENTITY_PERSISTENCE.md`. Current expected status is `pass` with `item_entity_persistence=rocksdb_guarded`, `storage_status=json_record_guarded`, `uncollected_drop_restart=live_server_guarded`, `pickup_after_restart=live_server_guarded`, `empty_after_pickup_restart=live_server_guarded`, `active_protocol_change=0`, and `active_chunk_payload_change=0`.
- For mining rules foundation checks, use `scripts/mining_rules_foundation_gate.sh`; see `docs/MINING_RULES_FOUNDATION.md`. Current expected status is `pass` with `mining_rules_status=cooldown_guarded`, `mining_block_durations=target_block_guarded`, `creative_default=unchanged`, `counted_mining_cooldown=server_guarded`, `active_protocol_change=0`, and `go_tests=pass`.
- For gameplay loop foundation checks, use `scripts/player_inventory_reconnect_smoke.sh`, `scripts/item_entity_persistence_gate.sh`, and `scripts/mining_rules_foundation_gate.sh` before `scripts/gameplay_loop_foundation_gate.sh`; see `docs/GAMEPLAY_LOOP_FOUNDATION.md`. Current expected status is `pass` with local hotbar inventory, selected slot state, authoritative inventory HUD labels/counts guarded, server session inventory guarded, `server_inventory_persistence=rocksdb_guarded`, `item_entity_persistence=rocksdb_guarded`, `uncollected_drop_restart=live_server_guarded`, `mining_rules_status=cooldown_guarded`, `mining_cooldown=server_guarded`, `mining_block_durations=target_block_guarded`, `player_inventory_reconnect=live_server_guarded`, required Block 41 persisted visual proof, `full_reload_persistence=block_41_visual_guarded`, and no protocol diff.
...
- For security/data-integrity review checks, use `scripts/security_data_integrity_review_gate.sh`; see `docs/SECURITY_DATA_INTEGRITY_REVIEW.md`. Current expected status is `pass` with packet framing, machine-readable deterministic packet/RLE property coverage, `packet_error_monitoring=export_ready`, `server_session_monitoring=export_ready`, `unknown_packet_policy=ignored_guarded`, `nil_packet_policy=ignored_guarded`, `nil_position_policy=ignored_guarded`, `nil_block_action_policy=ignored_guarded`, `conflict_semantics=last_write_wins_guarded`, `worldgen_biome_atlas_tile_identity=guarded`, `worldgen_biome_atlas_block_texture_usage=guarded`, `overload_status=admission_matrix_guarded`, `runtime_reconnect=repeated_live_rebootstrap_guarded`, `reconnect_smoke_status=pass`, `reconnect_soak_status=pass`, `slow_client_status=load_matrix_guarded`, `slow_reader_smoke_status=pass`, `slow_reader_matrix_status=pass`, `observability_gpu_report_freshness=guarded`, chunk decode, storage persistence/open-path behavior, `storage_package_smoke=guarded`, `storage_config=path_guarded`, `storage_backend_policy=approved_only_guarded`, `storage_backend_ownership=guarded`, `storage_concurrency=guarded`, `storage_errors=actionable_guarded`, `storage_lifecycle=guarded`, `block_edit_validation=y_bounds_guarded`, `block_edit_reach=position_guarded`, `block_edit_player_collision=place_player_aabb_guarded`, `block_edit_save_failure_rollback=guarded`, and `active_protocol_change=0`.
