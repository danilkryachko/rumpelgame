...
- The render shader lighting contract is guarded in Rust tests: vertex code computes lighting from `face_normal(face_idx)` and sanitized lighting push constants, passes it through `lighting_out`/`lighting_in`, and fragment code only applies that lighting to the sampled atlas texel with opaque alpha. Scene depth remains guarded separately as reverse-Z `GREATER_OR_EQUAL`.
