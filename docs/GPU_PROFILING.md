...
- GPU terrain atlas tile `col/row` is computed in the vertex shader and carried in `uv_tile_out.zw`; the fragment shader still applies `fract(tile_uv)` so merged-face texture repetition is preserved, but it no longer performs per-fragment tile-index `mod`/`floor`. Triangle corner selection also uses the global `TRIANGLE_CORNER_INDICES` table instead of declaring a local array in `main()`.
...
- The render shader contract is guarded in Rust tests: vertex code computes lighting from `face_normal(face_idx)` and sanitized lighting push constants, precomputes atlas tile offsets, passes lighting through `lighting_out`/`lighting_in`, and fragment code only applies repeated atlas UV plus lighting to the sampled atlas texel with opaque alpha. Scene depth remains guarded separately as reverse-Z `GREATER_OR_EQUAL`.
