...
- The native-shadow resource descriptor now records future draw submission markers as `native_shadow_draw_status=not_submitted`, `native_shadow_draw_call_count=0`, and `native_shadow_draw_face_count=0` for active descriptor tests; disabled resources report `none/0/0` and still do not submit native shadow draw calls or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
...
- The native-shadow resource descriptor now records future shader source/module contract markers as `native_shadow_shader_source_bytes=59` and `native_shadow_shader_source_checksum=3794449943` for active descriptor tests; disabled resources report `0/0` and still do not compile shader sources, allocate shader modules/RIDs, or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future shader module contract markers as `native_shadow_shader_module_status=source_ready` and `native_shadow_shader_module_rid_allocated=0` for active descriptor tests; disabled resources report `none/0` and still do not compile shader sources, allocate shader modules/RIDs, or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future pass/RID contract markers as `native_shadow_pass_status=descriptor_ready` and `native_shadow_pass_rid_allocated=0` for active descriptor tests; disabled resources report `none/0` and still do not allocate RenderingDevice pass/RID resources or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future pass submission boundary markers as `native_shadow_pass_submit_status=not_submitted`, `native_shadow_pass_begin_count=0`, and `native_shadow_pass_end_count=0` for active descriptor tests; disabled resources report `none/0/0` and still do not begin/end RenderingDevice passes or submit native shadow work while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future command buffer markers as `native_shadow_command_buffer_status=not_recorded`, `native_shadow_command_buffer_submit_count=0`, and `native_shadow_command_buffer_error_count=0` for active descriptor tests; disabled resources report `none/0/0` and still do not record or submit native shadow command buffers while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future depth attachment markers as `native_shadow_depth_attachment_status=not_bound`, `native_shadow_depth_attachment_binding_count=0`, and `native_shadow_depth_attachment_clear_count=0` for active descriptor tests; disabled resources report `none/0/0` and still do not bind or clear native shadow depth attachments while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future resource barrier markers as `native_shadow_resource_barrier_status=not_issued`, `native_shadow_resource_transition_count=0`, and `native_shadow_resource_barrier_error_count=0` for active descriptor tests; disabled resources report `none/0/0` and still do not issue native shadow resource barriers or transitions while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future framebuffer markers as `native_shadow_framebuffer_status=not_created`, `native_shadow_framebuffer_rid_allocated=0`, and `native_shadow_framebuffer_attachment_count=1` for active descriptor tests; disabled resources report `none/0/0` and still do not create native shadow framebuffers or RenderingDevice framebuffer RIDs while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future framebuffer/pass compatibility markers as `native_shadow_framebuffer_pass_compat_status=not_validated` and `native_shadow_framebuffer_pass_compat_error_count=0` for active descriptor tests; disabled resources report `none/0` and still do not validate native shadow framebuffer/pass compatibility, create framebuffers, or allocate RenderingDevice framebuffer RIDs while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future framebuffer depth-only markers as `native_shadow_framebuffer_depth_only_enabled=1` and `native_shadow_framebuffer_color_attachment_count=0` for active descriptor tests; disabled resources report `0/0` and still do not validate or create native shadow framebuffers, color attachments, or RenderingDevice framebuffer RIDs while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future framebuffer attachment lifetime markers as `native_shadow_framebuffer_attachment_owned=1` and `native_shadow_framebuffer_attachment_reuse_count=0` for active descriptor tests; disabled resources report `0/0` and still do not claim framebuffer attachment ownership, reuse attachments, create native shadow framebuffers, color attachments, or RenderingDevice framebuffer RIDs while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future framebuffer descriptor validation markers as `native_shadow_framebuffer_descriptor_valid=1` and `native_shadow_framebuffer_descriptor_error_count=0` for active descriptor tests; disabled resources report `0/0` and still do not validate framebuffer descriptors, create native shadow framebuffers, color attachments, or RenderingDevice framebuffer RIDs while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future framebuffer bind readiness markers as `native_shadow_framebuffer_bind_ready=1` and `native_shadow_framebuffer_bind_error_count=0` for active descriptor tests; disabled resources report `0/0` and still do not bind framebuffers to RenderDevice passes, validate framebuffer descriptors, create native shadow framebuffers, color attachments, or RenderingDevice framebuffer RIDs while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future pass descriptor validation markers as `native_shadow_pass_descriptor_valid=1` and `native_shadow_pass_descriptor_error_count=0` for active descriptor tests; disabled resources report `0/0` and still do not validate RenderDevice pass descriptors, allocate pass/RID resources, bind framebuffers to RenderDevice passes, or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future pass lifecycle readiness markers as `native_shadow_pass_lifecycle_ready=1` and `native_shadow_pass_lifecycle_error_count=0` for active descriptor tests; disabled resources report `0/0` and still do not begin/end RenderDevice passes, validate pass descriptors, allocate pass/RID resources, bind framebuffers to RenderDevice passes, or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future command buffer record readiness markers as `native_shadow_command_buffer_record_ready=1` and `native_shadow_command_buffer_record_error_count=0` for active descriptor tests; disabled resources report `0/0` and still do not record or submit native shadow command buffers, begin/end RenderDevice passes, allocate pass/RID resources, or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
- The native-shadow resource descriptor now records future command buffer submit readiness markers as `native_shadow_command_buffer_submit_ready=1` and `native_shadow_command_buffer_submit_error_count=0` for active descriptor tests; disabled resources report `0/0` and still do not submit or record native shadow command buffers, begin/end RenderDevice passes, allocate pass/RID resources, or run a native shadow pass while `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`.
