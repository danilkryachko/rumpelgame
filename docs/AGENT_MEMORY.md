...
- GPU-native terrain shadows remain behind `GPU_TERRAIN_NATIVE_SHADOW_IMPLEMENTED=false`. While runtime markers report `native_shadow_implemented=0`, env-on captures must stay on `shadow_path=godot_proxy` with `native_shadow_requested=1`, `native_shadow_active=0`, `native_shadow_fallback=1`, `native_shadow_resource_status=disabled`, and zero native-shadow resource lifecycle counters.
