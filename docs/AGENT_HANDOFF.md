...
- Current flyback stress slice adds visual-smoke motion `chunk_fly_out_back`, marker fields `current_chunk_loaded`, `current_chunk_submeshes`, `current_chunk_collision`, `ground_hit`, `ground_distance`, and `ground_y`, plus `scripts/gpu_terrain_flyback_stress.sh`. The wrapper is correctness-first: terrain/process/compositor budgets are report-only by default, while final chunk visibility, chunk collision, and a downward ground raycast under the return point are enforced.
- Fresh flyback artifact: `logs/gpu_flyback_stress` from `RUMPELMC_GODOT_RUST_EXT_BUILD_RELEASE=1 RUMPELMC_GODOT_RUST_EXT_PROFILE=release GODOT_QUIT_AFTER_FRAMES=36000 GODOT_TIMEOUT_SEC=300 SMOKE_DELAY_SEC=8.0 sh scripts/gpu_terrain_flyback_stress.sh logs/gpu_flyback_stress`. It passed with `motion_steps=13`, `motion_chunks=7`, `current_chunk="0,0"`, `current_chunk_loaded=1`, `current_chunk_submeshes=2`, `current_chunk_collision=2`, `ground_hit=1`, `ground_distance=12.000`, `ground_y=63.000`, `terrain_samples=381`, `terrain_queue_max_ms=2.453`, `gpu_compositor_submit_max_ms=0.228`, and `gpu_upload_fail=0`. The local server was stopped and port `25565` was cleared.
- Next useful stability step: run the flyback stress with `RUMPELMC_CLIENT_CHUNK_UNLOAD_GRACE_SEC=0` as a control/regression sample, then compare whether server chunk batch size or client position send interval still needs tuning.
