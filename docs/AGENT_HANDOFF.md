...
- Next useful GPU step: skip upload budget/retry/backoff and staging reuse for now because heavy radius-16 upload pressure is stable and the default upload breakdown is tiny. Continue with dirty-update telemetry, external GPU profiler setup, or capture-window hardening for the `extended-filled` case.
