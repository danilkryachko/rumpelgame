...
- Next useful stability step: add an automated fly-out/fly-back visual stress that asserts no terrain/collision holes around the return path, then compare whether server chunk batch size or client position send interval still needs tuning.
