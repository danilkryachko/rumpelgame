...
- Next useful GPU step: add a small block-edit stress/smoke that breaks or places blocks during capture so the new dirty telemetry records non-zero `dirty_blocks` and dirty subchunk bounds before implementing partial GPU dirty uploads.
