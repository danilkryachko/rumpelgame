...
- Current flyback stress slice adds visual-smoke motion `chunk_fly_out_back`, marker fields `current_chunk_loaded`, `current_chunk_submeshes`, `current_chunk_collision`, `ground_hit`, `ground_distance`, `ground_y`, `ground_samples`, `ground_hits`, `ground_misses`, `ground_max_distance`, and `ground_min_y`, plus `scripts/gpu_terrain_flyback_stress.sh`. The wrapper is correctness-first: terrain/process/compositor budgets are report-only by default, while final chunk visibility, chunk collision, and a 3x3 downward ground raycast grid around the return point are enforced.
- Fresh flyback artifact: `logs/gpu_flyback_stress_grid` from `RUMPELMC_GODOT_RUST_EXT_BUILD_RELEASE=1 RUMPELMC_GODOT_RUST_EXT_PROFILE=release GODOT_QUIT_AFTER_FRAMES=36000 GODOT_TIMEOUT_SEC=300 SMOKE_DELAY_SEC=8.0 sh scripts/gpu_terrain_flyback_stress.sh logs/gpu_flyback_stress_grid`. It passed with `motion_steps=13`, `motion_chunks=7`, `current_chunk="0,0"`, `current_chunk_loaded=1`, `current_chunk_submeshes=2`, `current_chunk_collision=2`, `ground_hit=1`, `ground_distance=12.000`, `ground_y=63.000`, `ground_samples=9`, `ground_hits=9`, `ground_misses=0`, `ground_max_distance=12.000`, `ground_min_y=63.000`, `terrain_samples=381`, `terrain_queue_max_ms=2.028`, `gpu_compositor_submit_max_ms=0.114`, and `gpu_upload_fail=0`. The local server was stopped and port `25565` was cleared.
- Control artifact: `logs/gpu_flyback_stress_grace0_no_settle_grid` from `RUMPELMC_CLIENT_CHUNK_UNLOAD_GRACE_SEC=0 RUMPELMC_FLYBACK_STRESS_SETTLE_SEC=0.0 ... sh scripts/gpu_terrain_flyback_stress.sh logs/gpu_flyback_stress_grace0_no_settle_grid` also passed the 3x3 ground grid with `ground_misses=0`, but logs show origin chunk unload/reload and higher residency churn (`queue=278`, `queue_max=307`, `queue_stale=30`, `gpu_draws=116`, `mem=81.0MB`) compared with the default grace run (`queue=0`, `queue_max=258`, `queue_stale=4`, `gpu_draws=324`, `mem=162.0MB`).
- Next useful stability step: add a harsher reproduction path for the screenshot hole: faster-than-position-send travel, diagonal/negative-z return, or an immediate post-teleport capture before server resend. A separate runtime safety guard should also prevent disabling noclip/falling when the local ground grid has misses.
