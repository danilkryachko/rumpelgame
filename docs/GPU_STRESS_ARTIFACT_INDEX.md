...
- shadow proxy refresh cost audit
...
The current index passed with `38` rows, `28` required rows, `28` required passes, `8` optional missing rows, zero upload-failure violations, zero ground-miss violations, zero default-runtime-change violations, and zero scheduler-change violations.
...
- Shadow proxy refresh cost audit: `case_count=18`, `matrix_case_count=16`, `pressure_case_count=2`, `max_proxy_shadow=243`, `max_proxy_shadow_only=228`, `max_compact_shadow_proxy=808`, `max_compact_shadow_normals_saved=5236`, `max_proxy_refresh_reuse=68`, `gpu_upload_fail=0`
...
- Shadow proxy refresh reuse: `68`
- Compact shadow proxy/savings max: `808` / `5236`
...
- `scripts/gpu_terrain_report.sh` surfaces the selected GPU shadow proxy refresh cost audit summary and case rows when present.
...
- `scripts/test_strategy_gate.sh` requires the shadow proxy refresh cost audit summary before the index.
