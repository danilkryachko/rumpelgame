...
10. Done 2026-06-14: added CPU-owned resident packed-face source bytes, deterministic compact payload preview, and explicit one-shot temporary replacement-buffer upload preview behind the repack flag, with source/payload/upload readiness telemetry and missing-source/source-size/upload-error validation; render binding swap remains intentionally unimplemented.
