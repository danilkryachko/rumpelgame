...
fn client_lifecycle_allows_outbound(state: ClientLifecycleState) -> bool {
...
pub struct GameClient {
    ...
    reconnect_event_count: u64,
    reconnect_attempt_count: u64,
    reconnect_success_count: u64,
    reconnect_retry_timer_sec: f64,
...
impl INode for GameClient {
    ...
    fn ready(&mut self) {
        ...
        if let Err(err) = self.connect_and_bootstrap_server() {
            self.emit_debug_log(&format!("Failed to connect to server: {err}"));
            self.record_client_lifecycle_event(ClientLifecycleEvent::ConnectFailed);
            self.reconnect_retry_timer_sec = RECONNECT_RETRY_INTERVAL_SEC;
        }
...
impl GameClient {
    fn record_client_lifecycle_event(&mut self, event: ClientLifecycleEvent) {
        if let Some(next_state) = client_lifecycle_transition(self.client_state, event) {
            if next_state != self.client_state {
                self.lifecycle_transition_count = self.lifecycle_transition_count.saturating_add(1);
                if next_state == ClientLifecycleState::Reconnecting {
                    self.reconnect_event_count = self.reconnect_event_count.saturating_add(1);
                    self.reconnect_retry_timer_sec = 0.0;
                }
            }
    ...
    fn record_network_reader_error(&mut self, error: String) {
        self.network_reader_errors = self.network_reader_errors.saturating_add(1);
    ...
    fn record_network_failure(&mut self, error: String) {
        ...
        self.emit_debug_log(&format!(
            "Network error; state={}",
            client_lifecycle_state_label(self.client_state)
        ));
    }

    fn drive_reconnect(&mut self, delta: f64) {
        if self.client_state != ClientLifecycleState::Reconnecting {
            return;
        }
        if delta.is_finite() && delta > 0.0 && self.reconnect_retry_timer_sec > 0.0 {
            self.reconnect_retry_timer_sec = (self.reconnect_retry_timer_sec - delta).max(0.0);
        }
        if self.reconnect_retry_timer_sec > 0.0 {
            return;
        }

        self.reconnect_attempt_count = self.reconnect_attempt_count.saturating_add(1);
        match self.connect_and_bootstrap_server() {
            Ok(()) => {
                self.reconnect_success_count = self.reconnect_success_count.saturating_add(1);
                self.last_network_error = "none".to_string();
                self.emit_debug_log("Reconnected to server; waiting for bootstrap chunk");
            }
            Err(err) => {
    ...
    fn connect_and_bootstrap_server(&mut self) -> Result<(), String> {
        let mut client =
            network::NetworkClient::connect(SERVER_ADDRESS).map_err(|err| err.to_string())?;
        let bootstrap_position = self.current_or_initial_position_packet();
        client
            .send_packet(&bootstrap_position)
            .map_err(|err| format!("send bootstrap position: {err}"))?;
    ...
    fn upload_gpu_subchunk(
            ...
            && let (Some(gpu_terrain), Some(packed_faces)) = (&mut self.gpu_terrain, packed_faces)
        {
            ...
            if uploaded {
                uploads = 1;
                bytes = upload_bytes;
                breakdown.record_upload(
                    existing_gpu_slot,
                    upload_bytes,
                    packed_faces.cutout_face_count(),
                    packed_faces.cutout_byte_len(),
                );
            }
    ...
    fn send_packet_to_server(&mut self, packet: &crate::api::Packet) {
        if !client_lifecycle_allows_outbound(self.client_state) {
            return;
        }
        ...
        if let Err(e) = network.send_packet(packet) {
            godot_print!("Failed to send packet: {}", e);
            self.record_network_failure(format!("send packet: {e}"));
        }
    }
...
struct GpuUploadBreakdown {
    ...
    cutout_uploads: usize,
    cutout_upload_bytes: usize,
    cutout_faces: usize,
    cutout_face_bytes: usize,
}
...
impl GpuUploadBreakdown {
    fn record_upload(
        ...
        if cutout_faces > 0 {
            self.cutout_uploads += 1;
            self.cutout_upload_bytes += bytes;
            self.cutout_faces += cutout_faces;
            self.cutout_face_bytes += cutout_face_bytes;
        }
    }
    ...
    fn add(&mut self, other: Self) {
        ...
        self.cutout_uploads += other.cutout_uploads;
        self.cutout_upload_bytes += other.cutout_upload_bytes;
        self.cutout_faces += other.cutout_faces;
        self.cutout_face_bytes += other.cutout_face_bytes;
    }
}
...
struct PerfStats {
    ...
    terrain_queue_gpu_cutout_slot_uploads: TerrainQueueUploadStats,
    terrain_queue_gpu_cutout_face_uploads: TerrainQueueUploadStats,
...
impl PerfStats {
    ...
    fn record_terrain_queue_frame_work(
        ...
        self.terrain_queue_gpu_cutout_slot_uploads.record_frame(
            gpu_upload_breakdown.cutout_uploads,
            gpu_upload_breakdown.cutout_upload_bytes,
            n,
        );
        self.terrain_queue_gpu_cutout_face_uploads.record_frame(
            gpu_upload_breakdown.cutout_faces,
            gpu_upload_breakdown.cutout_face_bytes,
            n,
        );
...
const GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_OWNED: u32 = 1;
const GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_REUSE_COUNT: u32 = 0;
const GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_VALID: u32 = 1;
const GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_ERROR_COUNT: u32 = 0;
const GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_READY: u32 = 1;
const GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_ERROR_COUNT: u32 = 0;
const GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_VALID: u32 = 1;
const GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_ERROR_COUNT: u32 = 0;
...
const GPU_TERRAIN_NATIVE_SHADOW_PASS_LIFECYCLE_READY: u32 = 1;
const GPU_TERRAIN_NATIVE_SHADOW_PASS_LIFECYCLE_ERROR_COUNT: u32 = 0;
...
const GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_RECORD_READY: u32 = 1;
const GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_RECORD_ERROR_COUNT: u32 = 0;
const GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_SUBMIT_READY: u32 = 1;
const GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_SUBMIT_ERROR_COUNT: u32 = 0;
...
struct GpuNativeShadowResourceDescriptor {
    ...
    framebuffer_attachment_owned: u32,
    framebuffer_attachment_reuse_count: u32,
    framebuffer_descriptor_valid: u32,
    framebuffer_descriptor_error_count: u32,
    framebuffer_bind_ready: u32,
    framebuffer_bind_error_count: u32,
    pass_descriptor_valid: u32,
    pass_descriptor_error_count: u32,
    pass_status: &'static str,
    pass_rid_allocated: u32,
    pass_submit_status: &'static str,
    pass_lifecycle_ready: u32,
    pass_lifecycle_error_count: u32,
    pass_begin_count: u32,
    pass_end_count: u32,
    command_buffer_status: &'static str,
    command_buffer_record_ready: u32,
    command_buffer_record_error_count: u32,
    command_buffer_submit_ready: u32,
    command_buffer_submit_error_count: u32,
...
impl GpuNativeShadowResourceDescriptor {
    fn new(shadow_radius_chunks: i32) -> Self {
        ...
        Self {
            ...
            framebuffer_attachment_owned: GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_OWNED,
            framebuffer_attachment_reuse_count:
                GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_REUSE_COUNT,
            framebuffer_descriptor_valid: GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_VALID,
            framebuffer_descriptor_error_count:
                GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_ERROR_COUNT,
            framebuffer_bind_ready: GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_READY,
            framebuffer_bind_error_count: GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_ERROR_COUNT,
            pass_descriptor_valid: GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_VALID,
            pass_descriptor_error_count: GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_ERROR_COUNT,
            ...
            pass_lifecycle_ready: GPU_TERRAIN_NATIVE_SHADOW_PASS_LIFECYCLE_READY,
            pass_lifecycle_error_count: GPU_TERRAIN_NATIVE_SHADOW_PASS_LIFECYCLE_ERROR_COUNT,
            ...
            command_buffer_record_ready: GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_RECORD_READY,
            command_buffer_record_error_count:
                GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_RECORD_ERROR_COUNT,
            command_buffer_submit_ready: GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_SUBMIT_READY,
            command_buffer_submit_error_count:
                GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_SUBMIT_ERROR_COUNT,
...
impl GpuNativeShadowResources {
    ...
    fn framebuffer_attachment_owned(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.framebuffer_attachment_owned)
            .unwrap_or(0)
    }

    fn framebuffer_attachment_reuse_count(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.framebuffer_attachment_reuse_count)
            .unwrap_or(0)
    }

    fn framebuffer_descriptor_valid(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.framebuffer_descriptor_valid)
            .unwrap_or(0)
    }

    fn framebuffer_descriptor_error_count(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.framebuffer_descriptor_error_count)
            .unwrap_or(0)
    }

    fn framebuffer_bind_ready(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.framebuffer_bind_ready)
            .unwrap_or(0)
    }

    fn framebuffer_bind_error_count(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.framebuffer_bind_error_count)
            .unwrap_or(0)
    }

    fn pass_descriptor_valid(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.pass_descriptor_valid)
            .unwrap_or(0)
    }

    fn pass_descriptor_error_count(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.pass_descriptor_error_count)
            .unwrap_or(0)
    }
    ...
    fn pass_lifecycle_ready(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.pass_lifecycle_ready)
            .unwrap_or(0)
    }

    fn pass_lifecycle_error_count(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.pass_lifecycle_error_count)
            .unwrap_or(0)
    }
    ...
    fn command_buffer_record_ready(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.command_buffer_record_ready)
            .unwrap_or(0)
    }

    fn command_buffer_record_error_count(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.command_buffer_record_error_count)
            .unwrap_or(0)
    }

    fn command_buffer_submit_ready(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.command_buffer_submit_ready)
            .unwrap_or(0)
    }

    fn command_buffer_submit_error_count(&self) -> u32 {
        self.descriptor
            .map(|descriptor| descriptor.command_buffer_submit_error_count)
            .unwrap_or(0)
    }
...
fn gpu_terrain_transparent_upload_counts(
    active: bool,
    cutout_requested: bool,
    gpu_stats: Option<gpu_terrain::GpuTerrainStats>,
) -> (u64, usize, usize, usize, usize, usize, usize) {
    if !active || !cutout_requested {
    ...
    (
        stats.cutout_upload_count,
        stats.cutout_upload_bytes,
        stats.cutout_upload_faces,
        stats.cutout_upload_face_bytes,
        stats.last_cutout_upload_bytes,
        stats.last_cutout_upload_faces,
        stats.last_cutout_upload_face_bytes,
    )
}

#[derive(Clone, Copy, Debug, PartialEq)]
struct TransparentCostMetrics {
    sort_policy: &'static str,
    sort_active: bool,
    sort_keys: u32,
    sort_ms: f64,
    build_cost_source: &'static str,
    build_faces: u32,
    build_subchunks: u32,
    build_envelope_ms: f64,
    build_uploads: u64,
    build_upload_bytes: usize,
    build_upload_faces: usize,
    build_upload_face_bytes: usize,
}

impl TransparentCostMetrics {
    fn inactive() -> Self {
        Self {
            sort_policy: "none",
            sort_active: false,
            sort_keys: 0,
            sort_ms: 0.0,
            build_cost_source: "inactive",
            build_faces: 0,
            build_subchunks: 0,
            build_envelope_ms: 0.0,
            build_uploads: 0,
            build_upload_bytes: 0,
            build_upload_faces: 0,
            build_upload_face_bytes: 0,
        }
    }
}
...
fn gpu_terrain_transparent_cost_metrics(
    active: bool,
    cutout_requested: bool,
    transparent_faces: u32,
    transparent_subchunks: u32,
    transparent_cutout_uploads: u64,
    transparent_cutout_upload_bytes: usize,
    transparent_cutout_upload_faces: usize,
    transparent_cutout_upload_face_bytes: usize,
    mesh_work_envelope_ms: f64,
) -> TransparentCostMetrics {
    if !active || !cutout_requested {
        return TransparentCostMetrics::inactive();
    }

    TransparentCostMetrics {
        sort_policy: "opaque_depth_alpha_test_no_sort",
        sort_active: false,
        sort_keys: 0,
        sort_ms: 0.0,
        build_cost_source: "cutout_in_opaque_mesh_phase",
        build_faces: transparent_faces,
        build_subchunks: transparent_subchunks,
        build_envelope_ms: mesh_work_envelope_ms,
        build_uploads: transparent_cutout_uploads,
        build_upload_bytes: transparent_cutout_upload_bytes,
        build_upload_faces: transparent_cutout_upload_faces,
        build_upload_face_bytes: transparent_cutout_upload_face_bytes,
    }
}
...
fn metric_text_label(value: &str) -> String {
...
impl GameClient {
        ...
        self
            ...
            .map(|pool| {
                ...
                format!(
                    ...
                    stats.draw_grouped_enabled,
                    stats.draw_records_logical,
                    stats.draw_records_grouped,
                    stats.draw_grouped_saved_records,
                    rasterization.cull_mode,
                    rasterization.front_face,
                    stats.faces,
                    stats.compositor_frames,
                    stats.compositor_breadcrumb,
                    stats.profiler_shader_label,
                    stats.compositor_pipeline_label,
                    ...
                    stats.upload_retry_giveups,
                    stats.upload_backoff_active,
                    stats.upload_backoff_frames,
                    stats.upload_backoff_max_frames,
                    stats.in_place_upload_enabled,
                    stats.in_place_uploads,
                    stats.in_place_upload_misses,
                    stats.upload_bytes as f64 / (1024.0 * 1024.0),
                    stats.last_upload_bytes as f64 / 1024.0,
                    stats.upload_staging_buffer_count,
                    stats.upload_staging_bytes as f64 / (1024.0 * 1024.0),
                    stats.last_upload_staging_bytes as f64 / 1024.0,
                    stats.last_upload_staging_bytes as f64 / 1024.0,
                    stats.avg_upload_staging_bytes / 1024.0,
                    stats.max_upload_staging_bytes as f64 / 1024.0,
                    ...
                    stats.upload_stage_pool_enabled,
                    stats.upload_stage_pool_entries,
                    stats.upload_stage_pool_bytes,
                    stats.upload_stage_pba_creates,
                    stats.upload_stage_pba_reuses,
                    stats.free_ranges,
                    stats.free_faces,
                    stats.largest_free_faces,
                    stats.fragmented_free_faces,
                    stats.fragmentation_pct,
                    stats.repack_requested,
                    stats.repack_active,
                    stats.repack_attempts,
                    stats.repack_success,
                    stats.repack_abort,
                    stats.repack_moved_subchunks,
                    stats.repack_moved_faces,
                    stats.repack_bytes,
                    stats.repack_source_subchunks,
                    stats.repack_source_bytes,
                    stats.repack_source_missing,
                    stats.repack_payload_ready,
                    stats.repack_payload_bytes,
                    stats.repack_upload_ready,
                    stats.repack_upload_bytes,
                    stats.repack_upload_ms,
                    stats.repack_bind_ready,
                    stats.repack_bind_ms,
                    stats.repack_draw_ready,
                    stats.repack_draw_bytes,
                    stats.repack_stage_ready,
                    stats.repack_stage_slots,
                    stats.repack_stage_bytes,
                    stats.repack_commit_ready,
                    stats.repack_commit_steps,
                    stats.repack_commit_tail_free,
                    stats.repack_apply_ready,
                    stats.repack_apply_steps,
                    stats.repack_apply_slots,
                    stats.repack_final_swap_ready,
                    stats.repack_final_swap_blocked,
                    stats.repack_final_swap_slots,
                    stats.repack_ms,
                    stats.repack_fragmentation_before_pct,
                    stats.repack_fragmentation_after_pct,
                    stats.repack_largest_free_before,
                    stats.repack_largest_free_after,
                    stats.repack_failure_reason,
    ...
    fn get_perf_text(&self) -> GString {
        ...
        let native_shadow_framebuffer_color_attachment_count = self
            .gpu_native_shadow_resources
            .framebuffer_color_attachment_count();
        let native_shadow_framebuffer_attachment_owned = self
            .gpu_native_shadow_resources
            .framebuffer_attachment_owned();
        let native_shadow_framebuffer_attachment_reuse_count = self
            .gpu_native_shadow_resources
            .framebuffer_attachment_reuse_count();
        let native_shadow_framebuffer_descriptor_valid = self
            .gpu_native_shadow_resources
            .framebuffer_descriptor_valid();
        let native_shadow_framebuffer_descriptor_error_count = self
            .gpu_native_shadow_resources
            .framebuffer_descriptor_error_count();
        let native_shadow_framebuffer_bind_ready =
            self.gpu_native_shadow_resources.framebuffer_bind_ready();
        let native_shadow_framebuffer_bind_error_count = self
            .gpu_native_shadow_resources
            .framebuffer_bind_error_count();
        let native_shadow_pass_descriptor_valid =
            self.gpu_native_shadow_resources.pass_descriptor_valid();
        let native_shadow_pass_descriptor_error_count = self
            .gpu_native_shadow_resources
            .pass_descriptor_error_count();
        ...
        let native_shadow_pass_lifecycle_ready =
            self.gpu_native_shadow_resources.pass_lifecycle_ready();
        let native_shadow_pass_lifecycle_error_count = self
            .gpu_native_shadow_resources
            .pass_lifecycle_error_count();
        ...
        let native_shadow_command_buffer_record_ready = self
            .gpu_native_shadow_resources
            .command_buffer_record_ready();
        let native_shadow_command_buffer_record_error_count = self
            .gpu_native_shadow_resources
            .command_buffer_record_error_count();
        let native_shadow_command_buffer_submit_ready = self
            .gpu_native_shadow_resources
            .command_buffer_submit_ready();
        let native_shadow_command_buffer_submit_error_count = self
            .gpu_native_shadow_resources
            .command_buffer_submit_error_count();
        ...
        let (
            ...
            transparent_cutout_last_upload_bytes,
            transparent_cutout_last_upload_faces,
            transparent_cutout_last_upload_face_bytes,
        ) = gpu_terrain_transparent_upload_counts(
            transparent_active,
            transparent_cutout_requested,
            gpu_terrain_stats,
        );
        let transparent_cost = gpu_terrain_transparent_cost_metrics(
            transparent_active,
            transparent_cutout_requested,
            transparent_faces,
        ...
        let text = format!(
            ...
            native_shadow_framebuffer_attachment_owned,
            native_shadow_framebuffer_attachment_reuse_count,
            native_shadow_framebuffer_descriptor_valid,
            native_shadow_framebuffer_descriptor_error_count,
            native_shadow_framebuffer_bind_ready,
            native_shadow_framebuffer_bind_error_count,
            native_shadow_pass_descriptor_valid,
            native_shadow_pass_descriptor_error_count,
            native_shadow_pass_status,
            native_shadow_pass_rid_allocated,
            native_shadow_pass_submit_status,
            native_shadow_pass_lifecycle_ready,
            native_shadow_pass_lifecycle_error_count,
            native_shadow_pass_begin_count,
            native_shadow_pass_end_count,
            native_shadow_command_buffer_status,
            native_shadow_command_buffer_record_ready,
            native_shadow_command_buffer_record_error_count,
            native_shadow_command_buffer_submit_ready,
            native_shadow_command_buffer_submit_error_count,
            ...
            transparent_cutout_last_upload_faces,
            transparent_cutout_last_upload_face_bytes,
            transparent_cost.sort_policy,
            transparent_cost.sort_active as u8,
            transparent_cost.sort_keys,
            transparent_cost.sort_ms,
            transparent_cost.build_cost_source,
            transparent_cost.build_faces,
            transparent_cost.build_subchunks,
            transparent_cost.build_envelope_ms,
            transparent_cost.build_uploads,
            transparent_cost.build_upload_bytes,
            transparent_cost.build_upload_faces,
            transparent_cost.build_upload_face_bytes,
            ...
            self.perf.terrain_queue_gpu_cutout_slot_uploads.last_uploads,
            self.perf.terrain_queue_gpu_cutout_slot_uploads.avg_uploads,
            self.perf.terrain_queue_gpu_cutout_slot_uploads.max_uploads,
            self.perf.terrain_queue_gpu_cutout_slot_uploads.last_bytes as f64 / 1024.0,
            self.perf.terrain_queue_gpu_cutout_slot_uploads.avg_bytes / 1024.0,
            self.perf.terrain_queue_gpu_cutout_slot_uploads.max_bytes as f64 / 1024.0,
            self.perf.terrain_queue_gpu_cutout_face_uploads.last_uploads,
            self.perf.terrain_queue_gpu_cutout_face_uploads.avg_uploads,
            self.perf.terrain_queue_gpu_cutout_face_uploads.max_uploads,
            self.perf.terrain_queue_gpu_cutout_face_uploads.last_bytes as f64 / 1024.0,
            self.perf.terrain_queue_gpu_cutout_face_uploads.avg_bytes / 1024.0,
            self.perf.terrain_queue_gpu_cutout_face_uploads.max_bytes as f64 / 1024.0,
...
mod tests {
    ...
    #[test]
    fn client_lifecycle_rebootstrap_returns_to_active() {
        let mut state = ClientLifecycleState::Active;

        state = client_lifecycle_transition(state, ClientLifecycleEvent::NetworkError).unwrap();
        assert_eq!(state, ClientLifecycleState::Reconnecting);
    ...
    #[test]
    fn client_lifecycle_outbound_is_active_only() {
        assert!(!client_lifecycle_allows_outbound(
            ClientLifecycleState::Connecting
        ));
        assert!(!client_lifecycle_allows_outbound(
            ClientLifecycleState::WaitingChunks
    ...
    #[test]
    fn metric_text_label_replaces_whitespace_and_punctuation() {
        assert_eq!(metric_text_label(""), "none");
        assert_eq!(
            metric_text_label("failed to fill whole buffer"),
            "failed_to_fill_whole_buffer"
        );
        assert_eq!(metric_text_label("read: EOF"), "read:_EOF");
    }
        ...
        assert_eq!(
            ...
            Some(GpuNativeShadowResourceDescriptor {
                ...
                framebuffer_color_attachment_count:
                    GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_COLOR_ATTACHMENT_COUNT,
                framebuffer_attachment_owned:
                    GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_OWNED,
                framebuffer_attachment_reuse_count:
                    GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_REUSE_COUNT,
                framebuffer_descriptor_valid:
                    GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_VALID,
                framebuffer_descriptor_error_count:
                    GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_ERROR_COUNT,
                framebuffer_bind_ready: GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_READY,
                framebuffer_bind_error_count:
                    GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_ERROR_COUNT,
                pass_descriptor_valid: GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_VALID,
                pass_descriptor_error_count: GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_ERROR_COUNT,
                pass_status: GPU_TERRAIN_NATIVE_SHADOW_PASS_STATUS,
                pass_rid_allocated: GPU_TERRAIN_NATIVE_SHADOW_PASS_RID_ALLOCATED,
    ...
    fn native_shadow_resource_lifecycle_tracks_create_reuse_replace_and_release() {
        ...
        assert_eq!(resources.framebuffer_attachment_owned(), 0);
        assert_eq!(resources.framebuffer_attachment_reuse_count(), 0);
        assert_eq!(resources.framebuffer_descriptor_valid(), 0);
        assert_eq!(resources.framebuffer_descriptor_error_count(), 0);
        assert_eq!(resources.framebuffer_bind_ready(), 0);
        assert_eq!(resources.framebuffer_bind_error_count(), 0);
        assert_eq!(resources.pass_descriptor_valid(), 0);
        assert_eq!(resources.pass_descriptor_error_count(), 0);
        ...
        assert_eq!(resources.pass_lifecycle_ready(), 0);
        assert_eq!(resources.pass_lifecycle_error_count(), 0);
        ...
        assert_eq!(resources.command_buffer_record_ready(), 0);
        assert_eq!(resources.command_buffer_record_error_count(), 0);
        assert_eq!(resources.command_buffer_submit_ready(), 0);
        assert_eq!(resources.command_buffer_submit_error_count(), 0);
        ...
        assert_eq!(
            resources.framebuffer_attachment_owned(),
            GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_OWNED
        );
        assert_eq!(
            resources.framebuffer_attachment_reuse_count(),
            GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_ATTACHMENT_REUSE_COUNT
        );
        assert_eq!(
            resources.framebuffer_descriptor_valid(),
            GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_VALID
        );
        assert_eq!(
            resources.framebuffer_descriptor_error_count(),
            GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_DESCRIPTOR_ERROR_COUNT
        );
        assert_eq!(
            resources.framebuffer_bind_ready(),
            GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_READY
        );
        assert_eq!(
            resources.framebuffer_bind_error_count(),
            GPU_TERRAIN_NATIVE_SHADOW_FRAMEBUFFER_BIND_ERROR_COUNT
        );
        assert_eq!(
            resources.pass_descriptor_valid(),
            GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_VALID
        );
        assert_eq!(
            resources.pass_descriptor_error_count(),
            GPU_TERRAIN_NATIVE_SHADOW_PASS_DESCRIPTOR_ERROR_COUNT
        );
        assert_eq!(
        ...
        assert_eq!(
            resources.pass_lifecycle_ready(),
            GPU_TERRAIN_NATIVE_SHADOW_PASS_LIFECYCLE_READY
        );
        assert_eq!(
            resources.pass_lifecycle_error_count(),
            GPU_TERRAIN_NATIVE_SHADOW_PASS_LIFECYCLE_ERROR_COUNT
        );
        assert_eq!(
        ...
        assert_eq!(
            resources.command_buffer_record_ready(),
            GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_RECORD_READY
        );
        assert_eq!(
            resources.command_buffer_record_error_count(),
            GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_RECORD_ERROR_COUNT
        );
        assert_eq!(
            resources.command_buffer_submit_ready(),
            GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_SUBMIT_READY
        );
        assert_eq!(
            resources.command_buffer_submit_error_count(),
            GPU_TERRAIN_NATIVE_SHADOW_COMMAND_BUFFER_SUBMIT_ERROR_COUNT
        );
        assert_eq!(
    ...
    fn gpu_transparent_gate_keeps_legacy_fallback_and_allows_cutout_prototype() {
        ...
        assert_eq!(
            gpu_terrain_transparent_upload_counts(false, true, None),
            (0, 0, 0, 0, 0, 0, 0)
        );
        assert_eq!(
            gpu_terrain_transparent_upload_counts(true, false, None),
            (0, 0, 0, 0, 0, 0, 0)
        );
        assert_eq!(
            gpu_terrain_transparent_upload_counts(
                ...
                true,
                Some(gpu_terrain::GpuTerrainStats {
                    cutout_upload_count: 4,
                    cutout_upload_bytes: 4096,
                    cutout_upload_faces: 12,
                    cutout_upload_face_bytes: 192,
                    last_cutout_upload_bytes: 1024,
                    last_cutout_upload_faces: 3,
                    last_cutout_upload_face_bytes: 48,
                    ..Default::default()
                }),
            ),
            (4, 4096, 12, 192, 1024, 3, 48)
        );
        assert_eq!(
            gpu_terrain_transparent_cost_metrics(false, true, 12, 3, 4, 4096, 12, 192, 4.5),
            TransparentCostMetrics::inactive()
        );
        assert_eq!(
            gpu_terrain_transparent_cost_metrics(true, false, 12, 3, 4, 4096, 12, 192, 4.5),
            TransparentCostMetrics::inactive()
        );
        assert_eq!(
            gpu_terrain_transparent_cost_metrics(true, true, 12, 3, 4, 4096, 12, 192, 4.5),
            TransparentCostMetrics {
                sort_policy: "opaque_depth_alpha_test_no_sort",
                sort_active: false,
                sort_keys: 0,
                ...
                build_faces: 12,
                build_subchunks: 3,
                build_envelope_ms: 4.5,
                build_uploads: 4,
                build_upload_bytes: 4096,
                build_upload_faces: 12,
                build_upload_face_bytes: 192,
            }
        );
    ...
    fn perf_records_terrain_queue_frame_work() {
        ...
        first_uploads.record_upload(false, 4096, 0, 0);
        let mut second_uploads = GpuUploadBreakdown::default();
        second_uploads.record_upload(false, 4096, 5, 80);
        second_uploads.record_upload(true, 4096, 2, 32);
        second_uploads.record_upload(true, 4096, 0, 0);
        ...
        assert_eq!(perf.terrain_queue_gpu_cutout_slot_uploads.last_uploads, 0);
        assert!(
            (perf.terrain_queue_gpu_cutout_slot_uploads.avg_uploads - 2.0 / 3.0).abs() < 0.000_001
        );
        assert_eq!(perf.terrain_queue_gpu_cutout_slot_uploads.max_uploads, 2);
        assert_eq!(perf.terrain_queue_gpu_cutout_slot_uploads.last_bytes, 0);
        assert!(
            (perf.terrain_queue_gpu_cutout_slot_uploads.avg_bytes - (8192.0 / 3.0)).abs()
                < 0.000_001
        );
        assert_eq!(perf.terrain_queue_gpu_cutout_slot_uploads.max_bytes, 8192);
        assert_eq!(perf.terrain_queue_gpu_cutout_face_uploads.last_uploads, 0);
        assert!(
            (perf.terrain_queue_gpu_cutout_face_uploads.avg_uploads - 7.0 / 3.0).abs() < 0.000_001
        );
        assert_eq!(perf.terrain_queue_gpu_cutout_face_uploads.max_uploads, 7);
        assert_eq!(perf.terrain_queue_gpu_cutout_face_uploads.last_bytes, 0);
        assert!(
            (perf.terrain_queue_gpu_cutout_face_uploads.avg_bytes - (112.0 / 3.0)).abs()
                < 0.000_001
        );
        assert_eq!(perf.terrain_queue_gpu_cutout_face_uploads.max_bytes, 112);
    }

    #[test]
    fn gpu_upload_breakdown_separates_new_and_replacement_slots() {
        let mut breakdown = GpuUploadBreakdown::default();
        breakdown.record_upload(false, 128, 4, 64);
        breakdown.record_upload(true, 64, 0, 0);

        let mut extra = GpuUploadBreakdown::default();
        extra.record_upload(false, 32, 1, 16);
        extra.record_upload(true, 16, 2, 32);
        breakdown.add(extra);
        ...
        assert_eq!(breakdown.cutout_uploads, 3);
        assert_eq!(breakdown.cutout_upload_bytes, 176);
        assert_eq!(breakdown.cutout_faces, 7);
        assert_eq!(breakdown.cutout_face_bytes, 112);
    }
