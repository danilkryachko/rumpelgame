...
awk \
  ...
  $1 == "world_load_suite_case" {
    ...
    if (type == "movement") {
      ...
      stream_scheduler_mode = value_of("stream_scheduler_mode")
      stream_scheduler_active = value_of("stream_scheduler_active") + 0
      stream_scheduler_preview_mismatch = value_of("stream_scheduler_preview_mismatch") + 0
      stream_scheduler_direction_x = value_of("stream_scheduler_direction_x") + 0
      stream_scheduler_direction_z = value_of("stream_scheduler_direction_z") + 0
      mesh_scheduler_directional_ties = value_of("mesh_scheduler_directional_ties") + 0
      collision_scheduler_directional_ties = value_of("collision_scheduler_directional_ties") + 0
      stream_scheduler_fifo_fallbacks = value_of("stream_scheduler_fifo_fallbacks") + 0
      ...
      if (stream_scheduler_active > max_stream_scheduler_active) max_stream_scheduler_active = stream_scheduler_active
      if (stream_scheduler_preview_mismatch > max_stream_scheduler_preview_mismatch) max_stream_scheduler_preview_mismatch = stream_scheduler_preview_mismatch
      if (mesh_scheduler_directional_ties > max_mesh_scheduler_directional_ties) max_mesh_scheduler_directional_ties = mesh_scheduler_directional_ties
      if (collision_scheduler_directional_ties > max_collision_scheduler_directional_ties) max_collision_scheduler_directional_ties = collision_scheduler_directional_ties
      if (stream_scheduler_fifo_fallbacks > max_stream_scheduler_fifo_fallbacks) max_stream_scheduler_fifo_fallbacks = stream_scheduler_fifo_fallbacks
      ...
      printf("chunk_boundary_case label=%s type=%s status=%s motion=%s expected_chunk=%s current_chunk=%s min_chunks=%d motion_steps=%d motion_chunks=%d terrain_queue_max_ms=%.3f process_wall_p95_ms=%.3f gpu_compositor_submit_max_ms=%.3f gpu_effective_draws=%d gpu_upload_fail=%d packet_queue_max_drain=%d packet_queue_drained=%d packet_queue_lag_max_ms=%.3f packet_queue_decode_work_max_ms=%.3f stream_scheduler_mode=%s stream_scheduler_active=%d stream_scheduler_preview_mismatch=%d stream_scheduler_direction_x=%d stream_scheduler_direction_z=%d mesh_scheduler_directional_ties=%d collision_scheduler_directional_ties=%d stream_scheduler_fifo_fallbacks=%d chunk_unload_total=%d chunk_unload_grace_kept=%d chunk_unload_neighbor_refreshes=%d chunk_unload_max=%d popin_missing_chunks=%d popin_collision_missing_chunks=%d popin_missing_max=%d popin_collision_missing_max=%d current_render_ready=%d current_collision_ready=%d ground_misses=%d\n", label, type, status, motion, expected_chunk, current_chunk, min_chunks, motion_steps, motion_chunks, terrain_queue_max, process_wall_p95, gpu_submit_max, gpu_draws, upload_fail, packet_drain, packet_drained, packet_lag, packet_decode_work, stream_scheduler_mode, stream_scheduler_active, stream_scheduler_preview_mismatch, stream_scheduler_direction_x, stream_scheduler_direction_z, mesh_scheduler_directional_ties, collision_scheduler_directional_ties, stream_scheduler_fifo_fallbacks, unload_total, unload_grace_kept, unload_neighbor_refreshes, unload_max, popin_missing, popin_collision_missing, popin_missing_max, popin_collision_missing_max, render_ready, collision_ready, ground_misses_case)
