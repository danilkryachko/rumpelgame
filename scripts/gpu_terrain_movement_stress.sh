...
write_summary() {
  ...
  gpu_upload_staging_buffers="$(metric gpu_upload_staging_buffers "$marker_path")"
  gpu_upload_staging_mb="$(float_metric gpu_upload_staging_mb "$marker_path")"
  gpu_upload_staging_kb_avg="$(default_float "$(perf_triplet_value gpu_upload_staging_kb "$marker_path" 2)")"
  gpu_upload_staging_kb_max="$(default_float "$(perf_triplet_value gpu_upload_staging_kb "$marker_path" 3)")"
  ...
  test -n "$gpu_upload_staging_buffers" || fail "missing gpu_upload_staging_buffers in $marker_path"
  ...
  awk \
    ...
    -v gpu_upload_staging_buffers="${gpu_upload_staging_buffers:-0}" \
    -v gpu_upload_staging_mb="${gpu_upload_staging_mb:-0}" \
    -v gpu_upload_staging_kb_avg="$gpu_upload_staging_kb_avg" \
...
(
  ...
  "$TIMEOUT_BIN" "$GODOT_TIMEOUT_SEC" /usr/bin/env \
    ...
    RUMPELMC_VISUAL_SMOKE_BLOCK_EDIT_SEQUENCE="${RUMPELMC_VISUAL_SMOKE_BLOCK_EDIT_SEQUENCE:-}" \
