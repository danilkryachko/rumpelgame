...
for path in "$DESIGN_DOC" "$WORLDGEN_DOC" "$RESOURCE_DOC" "$BIOME_SUMMARY" "$RESOURCE_SUMMARY" "$CHUNK_SOURCE" "$GENERATOR_SOURCE" "$RESOURCE_SOURCE" "$WORLD_SOURCE" "$WORLD_TEST" "$GENERATOR_TEST" "$RESOURCE_TEST" "$CHUNK_ENCODING_TEST" "$HEIGHT_SMOKE_SCRIPT" "$HEIGHT_SMOKE_SUMMARY" "$SERVER_CMD_SOURCE" "$SERVER_CMD_TEST"; do
...
for token in \
  ...
  'This block is complete as a seed/version, opt-in height-generator, metadata-only biome/resource, and opt-in biome-height checkpoint'; do
...
awk \
  ...
  BEGIN {
    ...
    printf("world_generation_quality status=%s reason=%s quality_pass_status=%s worldgen_seed_version=%s worldgen_height_v1=%s worldgen_biome_height_v1=%s height_v1_serialization=%s biome_height_v1_serialization=%s resource_distribution=%s height_v1_live_smoke=%s active_generator_change=%d active_chunk_byte_change=%d runtime_quality_pass=%s coordinate_mapping=%s origin_chunk=%s flat_byte_hash=%s world_tests=%s biome_status=%s biome_sampler=%s biome_matrix=%s biome_runtime=%s biome_active_worldgen_change=%d biome_active_serialization_change=%d resource_status=%s resource_matrix=%s resource_runtime=%s resource_active_worldgen_change=%d resource_active_serialization_change=%d resource_protocol_change=%d height_smoke_status=%s height_smoke_encoding=%s design_doc=%s biome_summary=%s resource_summary=%s\n", status, reason, quality_pass_status, worldgen_seed_version, worldgen_height_v1, worldgen_biome_height_v1, height_v1_serialization, biome_height_v1_serialization, resource_distribution, height_v1_live_smoke, active_generator_change, active_chunk_byte_change, runtime_quality_pass, coordinate_mapping, origin_chunk, flat_byte_hash, world_tests, biome_status, biome_sampler, biome_matrix, biome_runtime, biome_worldgen_change, biome_serialization_change, resource_status, resource_matrix, resource_runtime, resource_worldgen_change, resource_serialization_change, resource_protocol_change, height_smoke_status, height_smoke_encoding, design_doc, biome_summary, resource_summary)
    if (status != "pass") {
      exit 1
    }
  }
