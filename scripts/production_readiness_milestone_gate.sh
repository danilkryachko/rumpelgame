...

{
  ...
  printf 'row=storage_protocol_integrity status=%s active_protocol_change=%s observability_gpu_report_freshness=%s storage_package_smoke=%s packet_error_monitoring=%s server_session_monitoring=%s storage_config=%s storage_backend_policy=%s block_edit_validation=%s block_edit_save_failure_rollback=%s unknown_packet_policy=%s nil_packet_policy=%s nil_position_policy=%s nil_block_action_policy=%s conflict_semantics=%s worldgen_biome_atlas_tile_identity=%s worldgen_biome_atlas_block_texture_usage=%s overload_status=%s local_server_exposure=%s smoke_bind_exposure=%s source=%s\n' "$security_status" "$security_protocol_change" "$security_observability_gpu_report_freshness" "$security_storage_package_smoke" "$security_packet_error_monitoring" "$security_server_session_monitoring" "$security_storage_config" "$security_storage_backend_policy" "$security_block_edit_validation" "$security_block_edit_save_failure_rollback" "$security_unknown_packet_policy" "$security_nil_packet_policy" "$security_nil_position_policy" "$security_nil_block_action_policy" "$security_conflict_semantics" "$security_worldgen_biome_atlas_tile_identity" "$security_worldgen_biome_atlas_block_texture_usage" "$security_overload_status" "$security_local_server_exposure" "$security_smoke_bind_exposure" "$SECURITY_SUMMARY"
