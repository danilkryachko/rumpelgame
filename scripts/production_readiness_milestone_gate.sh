...

{
  ...
  printf 'row=storage_protocol_integrity status=%s active_protocol_change=%s conflict_semantics=%s local_server_exposure=%s smoke_bind_exposure=%s source=%s\n' "$security_status" "$security_protocol_change" "$security_conflict_semantics" "$security_local_server_exposure" "$security_smoke_bind_exposure" "$SECURITY_SUMMARY"
  printf 'row=docs_reproducible_gates observability_status=%s architecture_status=%s handoff_status=%s rc_status=%s rc_security_deterministic_property_tests=%s rc_security_conflict_semantics=%s rc_security_local_server_exposure=%s rc_security_smoke_bind_exposure=%s\n' "$observability_status" "$arch_status" "$handoff_status" "$rc_status" "$rc_security_deterministic_property_tests" "$rc_security_conflict_semantics" "$rc_security_local_server_exposure" "$rc_security_smoke_bind_exposure"
...
awk \
  ...
  BEGIN {
    ...
    printf("production_readiness_milestone status=%s reason=%s production_readiness=%s stable_streaming=%s high_resident_set=%s predictable_performance=%s resource_upload_health=%s storage_protocol_integrity=%s docs_reproducible_gates=%s external_profiler=%s external_capture_readiness=%s external_rc_security_conflict_semantics=%s external_rc_security_local_server_exposure=%s external_rc_security_smoke_bind_exposure=%s native_shadow_direction=%s transparent_direction=%s live_release_checks=%s rc_security_deterministic_property_tests=%s security_conflict_semantics=%s security_local_server_exposure=%s security_smoke_bind_exposure=%s rc_security_conflict_semantics=%s rc_security_local_server_exposure=%s rc_security_smoke_bind_exposure=%s resident_gpu_draws=%d resident_gpu_subchunks=%d upload_effective_draws=%d memory_fragmentation_pct=%.3f report=%s rc_summary=%s external_summary=%s\n", status, reason, production_readiness, stable_streaming, high_resident_set, predictable_performance, resource_upload_health, storage_protocol_integrity, docs_reproducible_gates, external_profiler, external_capture_readiness, external_rc_security_conflict_semantics, external_rc_security_local_server_exposure, external_rc_security_smoke_bind_exposure, native_shadow_direction, transparent_direction, rc_live_checks, rc_security_deterministic_property_tests, security_conflict_semantics, security_local_server_exposure, security_smoke_bind_exposure, rc_security_conflict_semantics, rc_security_local_server_exposure, rc_security_smoke_bind_exposure, resident_gpu_draws, load_subchunks, upload_effective_draws, memory_fragmentation, report_path, rc_summary, external_summary)
    if (status != "pass") {
      exit 1
    }
  }
