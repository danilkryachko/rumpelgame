...
client_server_registry_identity="$(field_metric server_registry_identity "$client_summary")"
...
awk \
  ...
  -v client_server_registry_identity="${client_server_registry_identity:-missing}" \
  ...
  BEGIN {
    ...
    gates_ok = server_status == "pass" &&
      ...
      client_server_registry_identity == "guarded" &&
