...
CANDIDATE_RADIUS="${RUMPELMC_WORLD_STREAMING_BOOTSTRAP_COMPARE_CANDIDATE:-0}"
...
require_positive_summary_metric() {
  label="$1"
  key="$2"
  value="$3"
  test -n "$value" || fail "missing $label $key in movement summary"
  awk -v label="$label" -v key="$key" -v value="$value" '
    BEGIN {
      if (value <= 0.0) {
        printf("world_streaming_bootstrap_compare: %s %s %.3f must be positive\n", label, key, value) > "/dev/stderr"
        exit 1
      }
    }
  '
...
summarize_run() {
  ...
  require_positive_summary_metric "$label" startup_chunk_packet_ms "$startup_chunk_packet_ms"
  require_positive_summary_metric "$label" startup_chunk_inserted_ms "$startup_chunk_inserted_ms"
  ...
  require_positive_summary_metric "$label" startup_mesh_dispatched_ms "$startup_mesh_dispatched_ms"
  require_positive_summary_metric "$label" startup_first_mesh_ms "$startup_first_mesh_ms"
  require_positive_summary_metric "$label" startup_first_mesh_work_ms "$startup_first_mesh_work_ms"
...
base_startup_packet_reader_elapsed="$(metric startup_packet_reader_elapsed_ms "$BASE_SUMMARY")"
candidate_startup_packet_reader_elapsed="$(metric startup_packet_reader_elapsed_ms "$CANDIDATE_SUMMARY")"
base_startup_packet_queue_lag="$(metric startup_packet_queue_lag_ms "$BASE_SUMMARY")"
candidate_startup_packet_queue_lag="$(metric startup_packet_queue_lag_ms "$CANDIDATE_SUMMARY")"
...
candidate_startup_first_mesh_work="$(metric startup_first_mesh_work_ms "$CANDIDATE_SUMMARY")"
base_startup_first_mesh_collision_work="$(metric startup_first_mesh_collision_work_ms "$BASE_SUMMARY")"
candidate_startup_first_mesh_collision_work="$(metric startup_first_mesh_collision_work_ms "$CANDIDATE_SUMMARY")"
...
awk \
  ...
  -v base_startup_packet_reader_elapsed="$base_startup_packet_reader_elapsed" \
  -v candidate_startup_packet_reader_elapsed="$candidate_startup_packet_reader_elapsed" \
  -v base_startup_packet_queue_lag="$base_startup_packet_queue_lag" \
  -v candidate_startup_packet_queue_lag="$candidate_startup_packet_queue_lag" \
  ...
  -v candidate_startup_first_mesh_work="$candidate_startup_first_mesh_work" \
  -v base_startup_first_mesh_collision_work="$base_startup_first_mesh_collision_work" \
  -v candidate_startup_first_mesh_collision_work="$candidate_startup_first_mesh_collision_work" \
