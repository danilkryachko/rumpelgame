...
for token in \
  ...
  'Compatibility Rules' \
  'inventory_protocol_status=snapshot_guarded' \
...
require_token "$SERVER_SOURCE" 'inventorySnapshot(inventory playerinventory.Inventory)'
...
awk \
  ...
  BEGIN {
    ...
    reason = "ok"
    inventory_protocol_status = "snapshot_guarded"
    current_wire_contract = "block_action_and_inventory_snapshot"
    future_packet_tags = "packet_gt_4"
    ...
    printf("inventory_protocol_compatibility status=%s reason=%s inventory_protocol_status=%s active_schema_change=%d current_wire_contract=%s inventory_snapshot_schema=%s future_packet_tags=%s future_block_action_fields=%s protocol_generated_drift=%s server_inventory_status=%s gameplay_inventory_status=%s protocol_summary=%s server_inventory_summary=%s gameplay_summary=%s design_doc=%s\n", status, reason, inventory_protocol_status, schema_diff_count, current_wire_contract, inventory_snapshot_schema, future_packet_tags, future_block_action_fields, protocol_generated_drift, server_inventory_report, gameplay_inventory_status, protocol_summary, server_inventory_summary, gameplay_summary, design_doc)
    if (status != "pass") {
      exit 1
    }
  }
