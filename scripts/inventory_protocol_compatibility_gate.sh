...
for token in \
  ...
  'Compatibility Rules' \
  'inventory_protocol_status=tool_action_guarded' \
...
awk \
  ...
  BEGIN {
    ...
    reason = "ok"
    inventory_protocol_status = "tool_action_guarded"
    current_wire_contract = "block_action_inventory_snapshot_action_tool_action_and_player_id"
    ...
    printf("inventory_protocol_compatibility status=%s reason=%s inventory_protocol_status=%s active_schema_change=%d current_wire_contract=%s position_identity_schema=%s inventory_snapshot_schema=%s inventory_action_schema=%s tool_selection_schema=%s future_packet_tags=%s future_block_action_fields=%s protocol_generated_drift=%s server_inventory_status=%s gameplay_inventory_status=%s protocol_summary=%s server_inventory_summary=%s gameplay_summary=%s design_doc=%s\n", status, reason, inventory_protocol_status, schema_diff_count, current_wire_contract, position_identity_schema, inventory_snapshot_schema, inventory_action_schema, tool_selection_schema, future_packet_tags, future_block_action_fields, protocol_generated_drift, server_inventory_report, gameplay_inventory_status, protocol_summary, server_inventory_summary, gameplay_summary, design_doc)
    if (status != "pass") {
      exit 1
    }
  }
