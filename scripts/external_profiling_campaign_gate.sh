...

{
  ...
  printf 'rc_input status=%s live_checks=%s test_gpu_report_freshness=%s security_deterministic_property_tests=%s security_packet_error_monitoring=%s security_server_session_monitoring=%s security_storage_package_smoke=%s security_storage_config=%s security_storage_backend_policy=%s security_block_edit_validation=%s security_block_edit_save_failure_rollback=%s security_unknown_packet_policy=%s security_nil_packet_policy=%s security_nil_position_policy=%s security_nil_block_action_policy=%s security_conflict_semantics=%s security_overload_status=%s security_local_server_exposure=%s security_smoke_bind_exposure=%s summary=%s\n' \
    "$rc_status" \
    "$rc_live_checks" \
    "$rc_test_gpu_report_freshness" \
    "$rc_security_deterministic_property_tests" \
    "$rc_security_packet_error_monitoring" \
    "$rc_security_server_session_monitoring" \
    "$rc_security_storage_package_smoke" \
    "$rc_security_storage_config" \
    "$rc_security_storage_backend_policy" \
    "$rc_security_block_edit_validation" \
    "$rc_security_block_edit_save_failure_rollback" \
    "$rc_security_unknown_packet_policy" \
    "$rc_security_nil_packet_policy" \
    "$rc_security_nil_position_policy" \
    "$rc_security_nil_block_action_policy" \
    "$rc_security_conflict_semantics" \
    "$rc_security_overload_status" \
