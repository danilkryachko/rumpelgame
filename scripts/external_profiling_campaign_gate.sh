...

{
  ...
  printf 'rc_input status=%s live_checks=%s security_deterministic_property_tests=%s summary=%s\n' \
    "$rc_status" \
    "$rc_live_checks" \
...
awk \
  ...
  BEGIN {
    ...
    } else if (!(rc_status == "pass" && rc_visual_smoke == "summary_ready" && rc_perf_matrix == "summary_ready" && rc_security_deterministic_property_tests == "guarded")) {
      status = "fail"
