...

{
  ...
  printf 'rc_input status=%s live_checks=%s security_deterministic_property_tests=%s security_local_server_exposure=%s summary=%s\n' \
    "$rc_status" \
    "$rc_live_checks" \
...
awk \
  ...
  BEGIN {
    ...
    } else if (!(rc_status == "pass" && rc_visual_smoke == "summary_ready" && rc_perf_matrix == "summary_ready" && rc_security_deterministic_property_tests == "guarded" && rc_security_local_server_exposure == "loopback_default_guarded")) {
      status = "fail"
