...

{
  ...
  printf 'rc_input status=%s live_checks=%s test_gpu_report_freshness=%s security_deterministic_property_tests=%s security_observability_gpu_report_freshness=%s security_packet_error_monitoring=%s security_server_session_monitoring=%s security_storage_package_smoke=%s security_storage_config=%s security_storage_backend_policy=%s security_block_edit_validation=%s security_block_edit_save_failure_rollback=%s security_unknown_packet_policy=%s security_nil_packet_policy=%s security_nil_position_policy=%s security_nil_block_action_policy=%s security_conflict_semantics=%s security_worldgen_biome_atlas_tile_identity=%s security_worldgen_biome_atlas_block_texture_usage=%s security_overload_status=%s security_local_server_exposure=%s security_smoke_bind_exposure=%s summary=%s\n' \
    ...
    "$rc_security_worldgen_biome_atlas_tile_identity" \
    "$rc_security_worldgen_biome_atlas_block_texture_usage" \
