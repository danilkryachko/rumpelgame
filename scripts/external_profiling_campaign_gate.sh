...

{
  ...
  printf 'rc_input status=%s live_checks=%s security_deterministic_property_tests=%s security_storage_config=%s security_storage_backend_policy=%s security_block_edit_validation=%s security_block_edit_save_failure_rollback=%s security_unknown_packet_policy=%s security_nil_packet_policy=%s security_nil_position_policy=%s security_nil_block_action_policy=%s security_conflict_semantics=%s security_local_server_exposure=%s security_smoke_bind_exposure=%s summary=%s\n' \
    "$rc_status" \
    "$rc_live_checks" \
