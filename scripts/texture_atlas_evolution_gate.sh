...
require_token "$BLOCKS_SOURCE" "fn block_material_textures_reference_guarded_atlas_tiles"
...
awk \
  ...
  BEGIN {
    ...
    printf("texture_atlas_evolution status=%s reason=%s atlas_metadata_status=%s atlas_tile_identity=%s block_texture_usage=%s active_asset_change=%d shader_layout_change=%d atlas_width=%d atlas_height=%d tile_size_px=%d columns=%d rows=%d tile_capacity=%d max_texture_tile=%d packed_tile_capacity=%d sampler=nearest/clamp_to_edge alpha_policy=opaque_alpha_forced manifest_status=not_introduced block_material_status=%s block_material_active_schema_change=%d design_doc=%s block_material_summary=%s\n", status, reason, atlas_metadata_status, atlas_tile_identity, block_texture_usage, active_asset_change, shader_layout_change, atlas_width, atlas_height, tile_size, columns, rows, tile_capacity, max_texture_tile, packed_tile_capacity, block_material_status, block_material_schema_change, design_doc, block_material_summary)
    if (status != "pass") {
      exit 1
    }
  }
