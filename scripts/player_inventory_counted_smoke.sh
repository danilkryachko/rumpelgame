...
OUT_DIR="${1:-"$ROOT_DIR/logs/player_inventory_counted_smoke"}"
case "$OUT_DIR" in
  /*) ;;
...
SMOKE_PORT="${RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_PORT:-25570}"
SMOKE_ADDR="127.0.0.1:$SMOKE_PORT"
SMOKE_DB="$OUT_DIR/rocksdb"
SUMMARY_PATH="$OUT_DIR/player-inventory-counted-smoke-summary.txt"
BUILD_SERVER="${RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_BUILD_SERVER:-1}"
PLAYER_ID="${RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_PLAYER_ID:-counted_player}"
SELECTED_SLOT="${RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_SLOT:-0}"
PLACE_BLOCK_ID="${RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_BLOCK_ID:-1}"
EXPECTED_COUNT="${RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_EXPECTED_COUNT:-7}"
SERVER_PID=""

mkdir -p "$OUT_DIR"

fail() {
  echo "player_inventory_counted_smoke: $*" >&2
  exit 1
}
...
if [ -n "$listener" ]; then
  fail "port $SMOKE_PORT is already in use; choose another RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_PORT"
fi
...
case "$BUILD_SERVER" in
  ...
  1)
    ...
    ;;
  *)
    fail "unsupported RUMPELMC_PLAYER_INVENTORY_COUNTED_SMOKE_BUILD_SERVER=$BUILD_SERVER"
    ;;
esac
...
start_server "place"
place_summary="$(run_phase place -action place-expect -block-id "$PLACE_BLOCK_ID" -expect-count "$EXPECTED_COUNT")"
cleanup_server
...
{
  printf 'player_inventory_counted_smoke status=pass counted_inventory_runtime=live_server_guarded phases=2 place_status=pass verify_restart_status=pass player_id=%s selected_slot=%s placed_block_id=%s expected_count=%s server_restarts=1 protocol_change=0 db_path=%s\n' \
    "$PLAYER_ID" "$SELECTED_SLOT" "$PLACE_BLOCK_ID" "$EXPECTED_COUNT" "$SMOKE_DB"
