...
test -n "$startup_chunk_packet_ms" || fail "missing startup packet timing in $MOVEMENT_SUMMARY"
test -n "$startup_chunk_inserted_ms" || fail "missing startup chunk inserted timing in $MOVEMENT_SUMMARY"
...
test -n "$startup_mesh_dispatched_ms" || fail "missing startup mesh dispatched timing in $MOVEMENT_SUMMARY"
