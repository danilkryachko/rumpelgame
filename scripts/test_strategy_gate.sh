...
EDIT_VISUAL_PARITY_SUMMARY="${RUMPELMC_TEST_STRATEGY_EDIT_VISUAL_PARITY_SUMMARY:-"$ROOT_DIR/logs/gpu_edit_visual_parity_gate_current/gpu-edit-visual-parity-summary.txt"}"
WORLD_INTERACTION_CHECKPOINT_SUMMARY="${RUMPELMC_TEST_STRATEGY_WORLD_INTERACTION_CHECKPOINT_SUMMARY:-"$ROOT_DIR/logs/gpu_world_interaction_checkpoint_current/gpu-world-interaction-checkpoint-summary.txt"}"
MACOS_METAL_CAPTURE_PACK_SUMMARY="${RUMPELMC_TEST_STRATEGY_MACOS_METAL_CAPTURE_PACK_SUMMARY:-"$ROOT_DIR/logs/gpu_macos_metal_capture_pack_current/gpu-macos-metal-capture-pack-summary.txt"}"
WINDOWS_CAPTURE_PACK_SUMMARY="${RUMPELMC_TEST_STRATEGY_WINDOWS_CAPTURE_PACK_SUMMARY:-"$ROOT_DIR/logs/gpu_windows_capture_pack_current/gpu-windows-capture-pack-summary.txt"}"
...
test -x "$ROOT_DIR/scripts/gpu_edit_visual_parity_gate.sh" || fail "missing executable edit visual parity gate"
test -x "$ROOT_DIR/scripts/gpu_world_interaction_checkpoint.sh" || fail "missing executable world interaction checkpoint gate"
test -x "$ROOT_DIR/scripts/gpu_macos_metal_capture_pack.sh" || fail "missing executable macOS Metal capture pack gate"
test -x "$ROOT_DIR/scripts/gpu_windows_capture_pack.sh" || fail "missing executable Windows capture pack gate"
...
edit_visual_parity_status="$(require_status edit_visual_parity "$EDIT_VISUAL_PARITY_SUMMARY" status pass)"
world_interaction_checkpoint_status="$(require_status world_interaction_checkpoint "$WORLD_INTERACTION_CHECKPOINT_SUMMARY" status pass)"
macos_metal_capture_pack_status="$(require_status macos_metal_capture_pack "$MACOS_METAL_CAPTURE_PACK_SUMMARY" status pass)"
windows_capture_pack_status="$(require_status windows_capture_pack "$WINDOWS_CAPTURE_PACK_SUMMARY" status pass)"
...

{
  printf 'test_strategy_gate status=pass fast_command="%s" full_command="%s" nightly_runtime_command="%s" nightly_summary_command="%s" exploration_soak_status=%s rapid_camera_turn_status=%s chunk_boundary_status=%s chunk_unload_churn_status=%s repeated_edit_benchmark_status=%s border_edit_benchmark_status=%s partial_dirty_edge_matrix_status=%s collision_refresh_cost_audit_status=%s shadow_proxy_refresh_cost_audit_status=%s edit_burst_budget_status=%s edit_visual_parity_status=%s world_interaction_checkpoint_status=%s streaming_priority_audit_status=%s streaming_scheduler_prototype_status=%s streaming_scheduler_workload_matrix_status=%s streaming_scheduler_tie_probe_status=%s streaming_scheduler_decision_checkpoint_status=%s load_scaling_status=%s upload_pressure_status=%s resource_lifecycle_status=%s memory_budget_status=%s buffer_residency_budget_status=%s report_v2_status=%s gpu_report_freshness_status=%s gpu_report_freshness=%s gpu_report_error_scan=%s gpu_report_upload_fail=%s gpu_report_upload_fail_capacity=%s gpu_report_upload_fail_fragmented=%s baseline_governance_status=%s gpu_stress_index_status=%s exploration_soak_summary=%s rapid_camera_turn_summary=%s chunk_boundary_summary=%s chunk_unload_churn_summary=%s repeated_edit_benchmark_summary=%s border_edit_benchmark_summary=%s partial_dirty_edge_matrix_summary=%s collision_refresh_cost_audit_summary=%s shadow_proxy_refresh_cost_audit_summary=%s edit_burst_budget_summary=%s edit_visual_parity_summary=%s world_interaction_checkpoint_summary=%s streaming_priority_audit_summary=%s streaming_scheduler_prototype_summary=%s streaming_scheduler_workload_matrix_summary=%s streaming_scheduler_tie_probe_summary=%s streaming_scheduler_decision_checkpoint_summary=%s load_scaling_summary=%s upload_pressure_summary=%s resource_lifecycle_summary=%s memory_budget_summary=%s buffer_residency_budget_summary=%s report_v2_summary=%s gpu_report_freshness_summary=%s baseline_governance_summary=%s gpu_stress_index_summary=%s\n' \
    ...
    "$edit_visual_parity_status" \
    "$world_interaction_checkpoint_status" \
  ...
  printf 'test_strategy_required_external_profiler_step macos_metal_capture_pack_status=%s macos_metal_capture_pack_summary=%s command="%s"\n' \
    "$macos_metal_capture_pack_status" \
  ...
  printf 'test_strategy_required_external_profiler_step windows_capture_pack_status=%s windows_capture_pack_summary=%s command="%s"\n' \
    "$windows_capture_pack_status" \
    "$(relative_path "$WINDOWS_CAPTURE_PACK_SUMMARY")" \
