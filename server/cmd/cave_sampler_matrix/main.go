...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "cave_sampler_matrix status=fail error=%q\n", err)
		os.Exit(1)
	}
	fmt.Println(summary)
}
...
func buildSummary(seed int64, dimension string, generatorVersion world.GeneratorVersion) (string, error) {
	...
	for _, coord := range matrixCoords {
		sample := generator.SampleCave(coord[0], int(coord[1]), coord[2])
		counts[sample.ID]++
		signaturePart := fmt.Sprintf("%d:%d:%d:%s:%d", coord[0], coord[1], coord[2], sample.ID, sample.Density)
	...
	return fmt.Sprintf(
		"cave_sampler_matrix status=pass algorithm_version=%s seed=%d dimension=%s generator_version=%s samples=%d solid=%d open=%d sample_hash=%x signature=%s active_worldgen_change=0 active_serialization_change=0 protocol_change=0",
		world.CaveAlgorithmVersionV1,
		seed,
		dimension,
		generatorVersion,
		len(matrixCoords),
		counts[world.CaveSolid],
		counts[world.CaveOpen],
		sum,
